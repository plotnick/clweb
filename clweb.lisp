...
(DEFMETHOD WALK-ATOMIC-FORM ((WALKER WALKER) FORM ENV) (DECLARE (IGNORE ENV))
           FORM)
...
(DEFUN WALK-QUOTE (WALKER FORM ENV) (DECLARE (IGNORE WALKER ENV)) FORM)
...
(DEFUN VALID-FUNCTION-NAME-P (NAME)
  (TYPECASE NAME (CONS (EQL (CAR NAME) 'SETF)) (SYMBOL T) (OTHERWISE NIL)))
(DEFINE-SPECIAL-FORM-WALKER FUNCTION
    ((WALKER WALKER) FORM ENV)
  `(,(CAR FORM)
    ,(COND ((VALID-FUNCTION-NAME-P (CADR FORM)) (CADR FORM))
           ((AND (CONSP (CADR FORM)) (EQL (CAADR FORM) 'LAMBDA))
            (WALK-LAMBDA-EXPRESSION WALKER (CADR FORM) ENV))
           (T
            (CERROR "Walk the function name anyway." 'INVALID-FUNCTION-NAME
                    :NAME (CADR FORM))
...
(DEFUN WALK-LAMBDA-LIST
  ...
  (LABELS ((AUGMENT-ENV
               (&REST VARS
                &AUX (VARS (MAPCAN (LAMBDA (X) (AND X (LIST X))) VARS)))
             (SETQ ENV
...
(DEFUN WALK-LAMBDA-EXPRESSION
       (WALKER FORM ENV &AUX (LAMBDA-LIST (CADR FORM)) (BODY (CDDR FORM)))
...
(DEFUN MAKE-MACRO-DEFINITIONS (WALKER DEFS ENV)
  (MAPCAR
   (LAMBDA (DEF &AUX (NAME (WALK-ATOMIC-FORM WALKER (CAR DEF) ENV)))
     (LIST NAME
