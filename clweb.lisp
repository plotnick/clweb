...
(DEFPACKAGE "CLWEB" (:USE "COMMON-LISP" #+:SBCL "SB-CLTL2" #+:ALLEGRO "SYS" #+:CCL "CCL")
            (:EXPORT "TANGLE-FILE" "LOAD-WEB" "WEAVE" "LOAD-SECTIONS-FROM-TEMP-FILE" "*TANGLE-FILE-PATHNAME*"
             "*TANGLE-FILE-TRUENAME*" "*WEAVE-PRINT*" "*WEAVE-VERBOSE*" "*INDEX-LEXICAL-VARIABLES*"
             "AMBIGUOUS-PREFIX-ERROR" "SECTION-NAME-CONTEXT-ERROR" "SECTION-NAME-USE-ERROR"
             "SECTION-NAME-DEFINITION-ERROR" "UNUSED-NAMED-SECTION-WARNING")
            (:SHADOW "ENCLOSE" #+:ALLEGRO "FUNCTION-INFORMATION" #+:ALLEGRO "VARIABLE-INFORMATION"))
(IN-PACKAGE "CLWEB")
                  ...
                  (:REPORT
                   (LAMBDA (CONDITION STREAM)
                     (FORMAT STREAM "~@<Ambiguous prefix: <~A> matches both <~A> and <~A>~:@>"
                             (AMBIGUOUS-PREFIX CONDITION) (AMBIGUOUS-PREFIX-FIRST-MATCH CONDITION)
...
(DEFINE-CONDITION INVALID-FUNCTION-NAME (PARSE-ERROR) ((NAME :INITARG :NAME :READER INVALID-FUNCTION-NAME))
                  (:REPORT
                   (LAMBDA (ERROR STREAM)
                     (FORMAT STREAM "~@<Invalid function name ~A.~:@>" (INVALID-FUNCTION-NAME ERROR)))))
(DEFUN ENSURE-LIST (OBJECT) (IF (LISTP OBJECT) OBJECT (LIST OBJECT)))
(DEFUN MAPAPPEND (FUNCTION &REST ARGS)
  (IF (SOME #'NULL ARGS)
      NIL
    (APPEND (APPLY FUNCTION (MAPCAR #'CAR ARGS)) (APPLY #'MAPAPPEND FUNCTION (MAPCAR #'CDR ARGS)))))
...
(DEFMETHOD PUSH-SECTION ((SECTION SECTION))
  (SETF (SECTION-NUMBER SECTION) (VECTOR-PUSH-EXTEND SECTION *SECTIONS*)) SECTION)
         ...
         (IF NODE
           ...
           (IF INSERT-IF-NOT-FOUND
               (LET ((NODE (MAKE-INSTANCE (CLASS-OF ROOT) :KEY ITEM)))
                 (WHEN PARENT
                   (IF (LESSP ITEM PARENT) (SETF (LEFT-CHILD PARENT) NODE) (SETF (RIGHT-CHILD PARENT) NODE)))
                 (VALUES NODE NIL))
             (VALUES NIL NIL)))) )))
  ...
  (MULTIPLE-VALUE-BIND (NODE PRESENT-P)
    ...
    (IF PRESENT-P
        (OR (DOLIST (CHILD (LIST (LEFT-CHILD NODE) (RIGHT-CHILD NODE)))
            ...
            (VALUES NODE T))
      (VALUES NODE NIL))))
(UNLESS (FBOUNDP 'WHITESPACEP) (DEFUN WHITESPACEP (CHAR) (FIND CHAR *WHITESPACE* :TEST #'CHAR=)))
(DEFUN SQUEEZE (STRING)
  (LOOP WITH SQUEEZING = NIL FOR CHAR ACROSS (STRING-TRIM *WHITESPACE* STRING) IF (NOT SQUEEZING) IF (WHITESPACEP CHAR)
        DO (SETQ SQUEEZING T) AND COLLECT #\Space INTO CHARS ELSE COLLECT CHAR INTO CHARS ELSE UNLESS
        (WHITESPACEP CHAR) DO (SETQ SQUEEZING NIL) AND COLLECT CHAR INTO CHARS FINALLY (RETURN (COERCE CHARS 'STRING))))
...
(DEFMETHOD SHARED-INITIALIZE :AROUND ((INSTANCE CHARPOS-OUTPUT-STREAM) SLOT-NAMES &REST INITARGS &KEY STREAM)
  (APPLY #'CALL-NEXT-METHOD INSTANCE SLOT-NAMES
         (LIST* :PROXY
                (MAKE-BROADCAST-STREAM (MAKE-STRING-OUTPUT-STREAM :ELEMENT-TYPE (STREAM-ELEMENT-TYPE STREAM)) STREAM)
                INITARGS)))
...
(DEFUN STREAM-CHARPOS (STREAM)
  (CHARPOS (OR (GETHASH STREAM *CHARPOS-STREAMS*) (ERROR "Not tracking charpos for ~S" STREAM))))
...
(DEFUN MAKE-CHARPOS-INPUT-STREAM (INPUT-STREAM &KEY (CHARPOS 0))
  (MAKE-INSTANCE 'CHARPOS-INPUT-STREAM :STREAM
                 (CASE INPUT-STREAM ((T) *TERMINAL-IO*) ((NIL) *STANDARD-INPUT*) (OTHERWISE INPUT-STREAM)) :CHARPOS
                 CHARPOS))
(DEFUN MAKE-CHARPOS-OUTPUT-STREAM (OUTPUT-STREAM &KEY (CHARPOS 0))
  (MAKE-INSTANCE 'CHARPOS-OUTPUT-STREAM :STREAM
                 (CASE OUTPUT-STREAM ((T) *TERMINAL-IO*) ((NIL) *STANDARD-OUTPUT*) (OTHERWISE OUTPUT-STREAM)) :CHARPOS
                 CHARPOS))
...
(DEFMACRO WITH-REWIND-STREAM
    ((VAR STREAM &OPTIONAL (REWIND 'REWIND)) &BODY BODY &AUX (IN (GENSYM)) (OUT (GENSYM)) (CLOSING (GENSYM)))
...
(DEFMACRO READ-WITH-ECHO
    ((STREAM OBJECT ECHOED) &BODY BODY &AUX (OUT (GENSYM)) (ECHO (GENSYM)) (RAW-OUTPUT (GENSYM)) (LENGTH (GENSYM)))
...
(DEFUN LIST-READER (STREAM CHAR)
  (DECLARE (IGNORE CHAR))
  (LOOP WITH LIST = 'NIL WITH CHARPOS-LIST = 'NIL FOR N UPFROM 0 AND NEXT-CHAR = (PEEK-CHAR T STREAM T NIL T) AS
        CHARPOS = (STREAM-CHARPOS STREAM) UNTIL (CHAR= #\) NEXT-CHAR) IF (CHAR= #\. NEXT-CHAR) DO
...
(DEFCLASS QUOTE-MARKER (MARKER)
          ((QUOTE :READER QUOTE-MARKER-QUOTE :INITARG :QUOTE) (FORM :READER QUOTED-FORM :INITARG :FORM)))
...
(DEFCLASS SPLICING-COMMA (COMMA) ((MODIFIER :READER COMMA-MODIFIER :INITARG :MODIFIER)))
  ...
  (SET-MACRO-CHARACTER #\,
                       (LAMBDA (STREAM CHAR)
                         ...
                         (CASE (PEEK-CHAR NIL STREAM T NIL T)
                           ((#\@ #\.)
                            (MAKE-INSTANCE 'SPLICING-COMMA :MODIFIER (READ-CHAR STREAM T NIL T) :FORM
                                           (READ STREAM T NIL T)))
...
(DEFUN BRACKET (X)
  (TYPECASE X (SPLICING-COMMA (COMMA-FORM X)) (COMMA `(LIST ,(COMMA-FORM X))) (T `(LIST ,(BQ-PROCESS X)))))
(SET-TANGLE-DISPATCH 'BACKQUOTE (LAMBDA (STREAM OBJ) (FORMAT STREAM "`~W" (CADR OBJ))))
(SET-TANGLE-DISPATCH 'SPLICING-COMMA
                     (LAMBDA (STREAM OBJ) (FORMAT STREAM ",~C~W" (COMMA-MODIFIER OBJ) (COMMA-FORM OBJ))) 1)
(SET-TANGLE-DISPATCH 'COMMA (LAMBDA (STREAM OBJ) (FORMAT STREAM ",~W" (COMMA-FORM OBJ))))
...
(DEFMETHOD MARKER-VALUE ((MARKER ARRAY-MARKER))
  (LOOP WITH CONTENTS = (TANGLE (ARRAY-MARKER-INITIAL-CONTENTS MARKER)) REPEAT (ARRAY-MARKER-RANK MARKER) FOR SEQ =
        CONTENTS THEN (ELT SEQ 0) COLLECT (LENGTH SEQ) INTO DIMENSIONS FINALLY
...
(DEFMETHOD MARKER-VALUE ((MARKER STRUCTURE-MARKER))
  (LET ((*READTABLE* (READTABLE-FOR-MODE NIL)))
    (VALUES (READ-FROM-STRING
               (WRITE-TO-STRING MARKER :PPRINT-DISPATCH *TANGLE-PPRINT-DISPATCH* :PRETTY T :READABLY T)))))
...
(DEFMETHOD MARKER-VALUE ((MARKER READ-TIME-CONDITIONAL-MARKER))
  (IF *EVALUATING*
      (CALL-NEXT-METHOD)
    (MAKE-INSTANCE 'READ-TIME-CONDITIONAL :PLUSP (READ-TIME-CONDITIONAL-PLUSP MARKER) :TEST
                   (READ-TIME-CONDITIONAL-TEST MARKER) :FORM (READ-TIME-CONDITIONAL-FORM MARKER))))
              ...
              (LOOP (LET ((NEXT-INPUT (READ-MAYBE-NOTHING-PRESERVING-WHITESPACE STREAM NIL *EOF* NIL)))
                      (WHEN NEXT-INPUT
                        (TYPECASE (SETQ FORM (CAR NEXT-INPUT))
                          ...
                          (SECTION (GO COMMENTARY))
                          (START-CODE-MARKER
                           (CERROR "Start a new unnamed section with no commentary." 'SECTION-LACKS-COMMENTARY :STREAM
                                   STREAM)
                           (SETQ FORM (MAKE-INSTANCE 'SECTION))
                           (PUSH (FINISH-SECTION SECTION COMMENTARY CODE) SECTIONS) (CHECK-TYPE FORM SECTION)
                           (SETQ SECTION FORM COMMENTARY 'NIL CODE 'NIL))
    ...
    (TYPECASE FORM
      ...
      (T
       (MULTIPLE-VALUE-BIND (A CAR-EXPANDED-P)
         ...
         (MULTIPLE-VALUE-BIND (D CDR-EXPANDED-P)
             (TANGLE-1 (CDR FORM))
           (VALUES (IF (AND (EQL A (CAR FORM)) (EQL D (CDR FORM))) FORM (CONS A D))
...
(DEFUN LOAD-WEB
    (FILESPEC &KEY (VERBOSE *LOAD-VERBOSE*) (PRINT *LOAD-PRINT*) (IF-DOES-NOT-EXIST T) (EXTERNAL-FORMAT :DEFAULT))
...
(DEFUN LOAD-SECTIONS-FROM-TEMP-FILE (FILE APPEND &AUX (TRUENAME (PROBE-FILE FILE)))
  "Load web sections from FILE, then delete it."
  (WHEN TRUENAME
    (UNWIND-PROTECT
        (WITH-OPEN-FILE (STREAM TRUENAME :DIRECTION :INPUT) (LOAD-WEB-FROM-STREAM STREAM T :APPEND APPEND))
...
(DEFUN TESTS-FILE-PATHNAME (OUTPUT-FILE TYPE &KEY (TESTS-FILE NIL TESTS-FILE-SUPPLIED-P) &ALLOW-OTHER-KEYS)
  (IF TESTS-FILE
    ...
    (UNLESS TESTS-FILE-SUPPLIED-P
      (MERGE-PATHNAMES (MAKE-PATHNAME :NAME (CONCATENATE 'STRING (PATHNAME-NAME OUTPUT-FILE :CASE :COMMON) "-TESTS")
                         :TYPE TYPE
                         :CASE :COMMON)
                       OUTPUT-FILE))))
(DEFUN TANGLE-FILE
    (INPUT-FILE &REST ARGS &KEY OUTPUT-FILE TESTS-FILE (VERBOSE *COMPILE-VERBOSE*) (PRINT *COMPILE-PRINT*)
     ...
     (TESTS-FILE (APPLY #'TESTS-FILE-PATHNAME OUTPUT-FILE "LISP" ARGS)) (*TANGLE-FILE-PATHNAME* INPUT-FILE)
     (*TANGLE-FILE-TRUENAME* (TRUENAME *TANGLE-FILE-PATHNAME*)) (*READTABLE* *READTABLE*) (*PACKAGE* *PACKAGE*))
  ...
  (FLET ((WRITE-FORMS (SECTIONS OUTPUT-FILE)
           (WITH-OPEN-FILE (OUTPUT OUTPUT-FILE :DIRECTION :OUTPUT :IF-EXISTS :SUPERSEDE :EXTERNAL-FORMAT
                            EXTERNAL-FORMAT)
             (FORMAT OUTPUT ";;;; TANGLED WEB FROM \"~A\". DO NOT EDIT.~%" INPUT-FILE)
             (LET ((*EVALUATING* NIL) (*PRINT-PPRINT-DISPATCH* *TANGLE-PPRINT-DISPATCH*) (*PRINT-LEVEL* NIL))
    ...
    (MULTIPLE-VALUE-PROG1 (APPLY #'COMPILE-FILE LISP-FILE :ALLOW-OTHER-KEYS T ARGS)
      (WHEN TESTS-FILE (COMPILE-FILE TESTS-FILE :VERBOSE VERBOSE :PRINT PRINT :EXTERNAL-FORMAT EXTERNAL-FORMAT)))))
(DEFUN WEAVE
    (INPUT-FILE &REST ARGS &KEY OUTPUT-FILE TESTS-FILE (INDEX-FILE NIL INDEX-FILE-SUPPLIED-P) (VERBOSE *WEAVE-VERBOSE*)
     (PRINT *WEAVE-PRINT*) (IF-DOES-NOT-EXIST T) (EXTERNAL-FORMAT :DEFAULT) &AUX
     (INPUT-FILE (MERGE-PATHNAMES INPUT-FILE (MAKE-PATHNAME :TYPE "CLW" :CASE :COMMON)))
     (OUTPUT-FILE
      (IF OUTPUT-FILE
     ...
     (INDEX-FILE
      (IF INDEX-FILE
        ...
        (WHEN (NOT INDEX-FILE-SUPPLIED-P) (MERGE-PATHNAMES (MAKE-PATHNAME :TYPE "IDX" :CASE :COMMON) OUTPUT-FILE))))
     (SECTIONS-FILE (WHEN INDEX-FILE (MERGE-PATHNAMES (MAKE-PATHNAME :TYPE "SCN" :CASE :COMMON) INDEX-FILE)))
  ...
  (SETQ *INDEX-PACKAGES* NIL)
  (SETQ *INDEX* (MAKE-INDEX))
  (WITH-OPEN-FILE (INPUT INPUT-FILE :DIRECTION :INPUT :EXTERNAL-FORMAT EXTERNAL-FORMAT :IF-DOES-NOT-EXIST
...
(DEFUN WEAVE-SECTIONS
    (SECTIONS &KEY INPUT-FILE OUTPUT-FILE INDEX-FILE SECTIONS-FILE TESTS (VERBOSE *WEAVE-VERBOSE*)
  ...
  (FLET ((WEAVE (OBJECT STREAM)
    ...
    (MACROLET ((WITH-OUTPUT-FILE ((STREAM FILESPEC) &BODY BODY)
                 `(WITH-OPEN-FILE (,STREAM ,FILESPEC :DIRECTION :OUTPUT :EXTERNAL-FORMAT EXTERNAL-FORMAT :IF-EXISTS
                                   :SUPERSEDE)
                    ,@BODY)))
      (WITH-OUTPUT-FILE (OUT OUTPUT-FILE) (FORMAT OUT "\\input clwebmac~%")
       ...
       (IF PRINT
           (FLET ((WEAVE (SECTION)
                    ...
                    (WEAVE SECTION OUT)))
             (PPRINT-LOGICAL-BLOCK (NIL (COERCE SECTIONS 'LIST) :PER-LINE-PREFIX ";  ") (WEAVE (PPRINT-POP))
       ...
       (WHEN INDEX-FILE
         ...
         (WITH-OUTPUT-FILE (IDX INDEX-FILE) (WEAVE (INDEX-SECTIONS SECTIONS) IDX))
  ...
  (WITH-MODE :RESTRICTED
    (WITH-INPUT-FROM-STRING (STREAM INPUT-STRING)
      (LOOP FOR TEXT = (SNARF-UNTIL-CONTROL-CHAR STREAM #\|) FOR FORMS =
            (READ-PRESERVING-WHITESPACE STREAM NIL *EOF* NIL) IF (PLUSP (LENGTH TEXT)) COLLECT TEXT IF (EOF-P FORMS) DO
...
(DEFUN PRINT-SECTION (STREAM SECTION)
  (FORMAT STREAM "~&\\~:[M~*~;N{~D}~]{~D}" (STARRED-SECTION-P SECTION) (SECTION-DEPTH SECTION)
...
(DEFUN PRINT-ESCAPED
    (STREAM STRING &REST ARGS &AUX
     (STREAM (CASE STREAM ((T) *TERMINAL-IO*) ((NIL) *STANDARD-OUTPUT*) (OTHERWISE STREAM))))
  (DECLARE (IGNORE ARGS))
...
(DEFUN PRINT-STRING (STREAM STRING)
  (LOOP WITH *PRINT-ESCAPE-LIST* = `(("{*}" . #\\) ("\\" . "\\\\\\\\") ("\"" . "\\\\\"") ,@*PRINT-ESCAPE-LIST*) FOR
        LAST = 0 THEN
        (IF (CHAR= (ELT STRING (1- NEWLINE)) #\~) (POSITION-IF-NOT #'WHITESPACEP STRING :START NEWLINE) (1+ NEWLINE))
        FOR NEWLINE = (POSITION #\Newline STRING :START LAST) AS LINE = (SUBSEQ STRING LAST NEWLINE) DO
        (FORMAT STREAM "\\.{~:[~;\"~]~/clweb::print-escaped/~:[~;\"~]}" (ZEROP LAST) LINE (NULL NEWLINE)) WHILE NEWLINE
           ...
           (NEXT-LOGICAL-BLOCK (LIST)
             (DO* ((BLOCK 'NIL)
                  ...
                  ((OR (ENDP LIST) (AND (EQ LIST NEWLINE) NEXT-INDENT (< 0 NEXT-INDENT BLOCK-INDENT)))
                   (VALUES (IF (NOTANY #'NEWLINEP BLOCK) (NREVERSE BLOCK) (MAKE-LOGICAL-BLOCK (NREVERSE BLOCK)))
                           LIST))
...
(SET-WEAVE-DISPATCH 'COMMENT-MARKER
                    (LAMBDA (STREAM OBJ)
                      (FORMAT STREAM "\\C{~/clweb::print-TeX/}" (READ-TEX-FROM-STRING (COMMENT-TEXT OBJ)))))
(SET-WEAVE-DISPATCH 'BACKQUOTE (LAMBDA (STREAM OBJ) (FORMAT STREAM "\\`~W" (CADR OBJ))))
(SET-WEAVE-DISPATCH 'SPLICING-COMMA
                    (LAMBDA (STREAM OBJ) (FORMAT STREAM "\\CO{~C}~W" (COMMA-MODIFIER OBJ) (SLOT-VALUE OBJ 'FORM))) 1)
(SET-WEAVE-DISPATCH 'COMMA (LAMBDA (STREAM OBJ) (FORMAT STREAM "\\CO{}~W" (SLOT-VALUE OBJ 'FORM))))
...
(UNLESS (FBOUNDP 'ENSURE-PORTABLE-WALKING-ENVIRONMENT) (DEFUN ENSURE-PORTABLE-WALKING-ENVIRONMENT (ENV) ENV))
...
(DEFMETHOD WALK-FORM ((WALKER WALKER) FORM &OPTIONAL ENV &AUX (ENV (ENSURE-PORTABLE-WALKING-ENVIRONMENT ENV))
                      (EXPANDED T))
  (FLET ((SYMBOL-MACRO-P (FORM ENV) (AND (SYMBOLP FORM) (EQL (VARIABLE-INFORMATION FORM ENV) :SYMBOL-MACRO))))
    (LOOP (CATCH FORM
            (COND ((SYMBOL-MACRO-P FORM ENV))
                  ((ATOM FORM) (RETURN (WALK-ATOMIC-FORM WALKER :EVALUATED FORM ENV)))
...
(DEFMETHOD AUGMENT-WALKER-ENVIRONMENT ((WALKER WALKER) ENV &REST ARGS) (APPLY #'AUGMENT-ENVIRONMENT ENV ARGS))
(DEFUN WALK-LIST (WALKER LIST ENV)
  (DO ((FORM LIST (CDR FORM)) (NEWFORM NIL (CONS (WALK-FORM WALKER (CAR FORM) ENV) NEWFORM)))
      ((ATOM FORM) (NRECONC NEWFORM FORM)) ))
(DEFMETHOD WALK-ATOMIC-FORM ((WALKER WALKER) CONTEXT FORM ENV &KEY) (DECLARE (IGNORE CONTEXT ENV)) FORM)
(DEFMETHOD WALK-ATOMIC-FORM ((WALKER WALKER) CONTEXT (FORM CONS) ENV &KEY)
  (DECLARE (IGNORE ENV)) (ERROR "Unexpected non-atomic form ~S (~S)" FORM CONTEXT))
(DEFMETHOD WALK-COMPOUND-FORM ((WALKER WALKER) OPERATOR FORM ENV)
  (DECLARE (IGNORE OPERATOR))
  `(,(WALK-ATOMIC-FORM WALKER :OPERATOR (CAR FORM) ENV) ,@(WALK-LIST WALKER (CDR FORM) ENV)))
...
(DEFMETHOD WALK-COMPOUND-FORM ((WALKER WALKER) (OPERATOR CONS) FORM ENV)
  ...
  `(,(WALK-LAMBDA-EXPRESSION WALKER OPERATOR ENV) ,@(WALK-LIST WALKER (CDR FORM) ENV)))
(DEFTYPE SETF-FUNCTION-NAME () '(CONS (EQL SETF) (CONS SYMBOL NULL)))
(DEFUN SETF-FUNCTION-NAME-P (FUNCTION-NAME) (TYPEP FUNCTION-NAME 'SETF-FUNCTION-NAME))
(DEFMETHOD WALK-FUNCTION-NAME ((WALKER WALKER) FUNCTION-NAME ENV &KEY)
  (TYPECASE FUNCTION-NAME
    ...
    (SETF-FUNCTION-NAME `(SETF ,(WALK-ATOMIC-FORM WALKER :UNEVALUATED (CADR FUNCTION-NAME) ENV)))
    (T (CERROR "Use the function name anyway." 'INVALID-FUNCTION-NAME :NAME FUNCTION-NAME) FUNCTION-NAME)))
...
(DEFMACRO DEFINE-SPECIAL-FORM-WALKER
    (OPERATOR (WALKER FORM ENV &REST REST) &BODY BODY &AUX (OPARG `(,(GENSYM) (EQL ',OPERATOR))))
...
(MACROLET ((DEFINE-BLOCK-LIKE-WALKER (OPERATOR)
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR ((WALKER WALKER) FORM ENV)
                `(,(CAR FORM) ,(WALK-ATOMIC-FORM WALKER :UNEVALUATED (CADR FORM) ENV)
...
(MACROLET ((DEFINE-QUOTE-LIKE-WALKER (OPERATOR)
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR ((WALKER WALKER) FORM ENV) (DECLARE (IGNORE WALKER ENV)) FORM)))
  (DEFINE-QUOTE-LIKE-WALKER QUOTE)
  (DEFINE-QUOTE-LIKE-WALKER GO))
(DEFINE-SPECIAL-FORM-WALKER EVAL-WHEN ((WALKER WALKER) FORM ENV)
  `(,(CAR FORM) ,(WALK-LIST WALKER (CADR FORM) ENV) ,@(WALK-LIST WALKER (CDDR FORM) ENV)))
...
(DEFUN PARSE-BODY (BODY &KEY DOC-STRING-ALLOWED WALKER ENV &AUX DOC)
  (FLET ((DOC-STRING-P (X REMAINING-FORMS) (AND (STRINGP X) DOC-STRING-ALLOWED REMAINING-FORMS (NULL DOC)))
         (DECLARATION-P (X) (AND (LISTP X) (EQL (CAR X) 'DECLARE))))
    (LOOP FOR FORMS = BODY THEN (CDR FORMS) AS X = (CAR FORMS) WHILE FORMS IF (DOC-STRING-P X (CDR FORMS)) DO
          (SETQ DOC X) ELSE IF (DECLARATION-P X) APPEND (CDR X) INTO DECLS ELSE DO (LOOP-FINISH) FINALLY
          (RETURN (VALUES FORMS (IF WALKER (WALK-DECLARATION-SPECIFIERS WALKER DECLS ENV) DECLS) DOC)))))
(DEFMETHOD WALK-DECLARATION-SPECIFIERS ((WALKER WALKER) DECLS ENV) (DECLARE (IGNORE ENV)) DECLS)
(DEFUN WALK-LAMBDA-LIST (WALKER LAMBDA-LIST DECLS ENV &AUX NEW-LAMBDA-LIST (STATE :REQVARS))
  (LABELS ((AUGMENT-ENV (&REST VARS &AUX (VARS (REMOVE-IF #'NULL VARS)))
           ...
           (WALK-VAR (VAR) (WALK-ATOMIC-FORM WALKER :BINDING VAR ENV))
    ...
    (DO* ((LAMBDA-LIST LAMBDA-LIST (CDR LAMBDA-LIST)) (ARG (CAR LAMBDA-LIST) (CAR LAMBDA-LIST)))
         ((NULL LAMBDA-LIST) (VALUES (NREVERSE NEW-LAMBDA-LIST) ENV))
      (ECASE STATE
        (:ENVVAR (PUSH (WALK-VAR (POP LAMBDA-LIST)) NEW-LAMBDA-LIST)
         ...
         (ETYPECASE ARG
           (SYMBOL
            (COND ((MEMBER ARG LAMBDA-LIST-KEYWORDS) (PUSH ARG NEW-LAMBDA-LIST) (UPDATE-STATE ARG))
                  (T (SETQ ARG (WALK-VAR ARG)) (AUGMENT-ENV ARG) (PUSH ARG NEW-LAMBDA-LIST))))
        ...
        (:OPTVARS
         (ETYPECASE ARG
           (SYMBOL
            (COND ((MEMBER ARG LAMBDA-LIST-KEYWORDS) (PUSH ARG NEW-LAMBDA-LIST) (UPDATE-STATE ARG))
                  (T (SETQ ARG (WALK-VAR ARG)) (AUGMENT-ENV ARG) (PUSH ARG NEW-LAMBDA-LIST))))
           (CONS (DESTRUCTURING-BIND (VAR/PATTERN &OPTIONAL INIT-FORM SUPPLIED-P-PARAMETER)
                     ARG
                   (WHEN INIT-FORM (SETQ INIT-FORM (WALK-FORM WALKER INIT-FORM ENV)))
                   (MULTIPLE-VALUE-SETQ (VAR/PATTERN ENV) (MAYBE-DESTRUCTURE VAR/PATTERN))
                   (WHEN SUPPLIED-P-PARAMETER (AUGMENT-ENV SUPPLIED-P-PARAMETER))
                   (PUSH (NCONC (LIST VAR/PATTERN) (AND INIT-FORM (LIST INIT-FORM))
                                (AND SUPPLIED-P-PARAMETER (LIST SUPPLIED-P-PARAMETER)))
                         NEW-LAMBDA-LIST)))))
        (:KEYVARS
         (COND ((EQL ARG '&ALLOW-OTHER-KEYS) (PUSH (POP LAMBDA-LIST) NEW-LAMBDA-LIST) (UPDATE-STATE (CAR LAMBDA-LIST)))
               (T
                (ETYPECASE ARG
                  (SYMBOL
                   (COND ((MEMBER ARG LAMBDA-LIST-KEYWORDS) (PUSH ARG NEW-LAMBDA-LIST) (UPDATE-STATE ARG))
                         (T (SETQ ARG (WALK-VAR ARG)) (AUGMENT-ENV ARG) (PUSH ARG NEW-LAMBDA-LIST))))
                  (CONS (DESTRUCTURING-BIND (VAR/KV &OPTIONAL INIT-FORM SUPPLIED-P-PARAMETER)
                            ARG
                          (WHEN INIT-FORM (SETQ INIT-FORM (WALK-FORM WALKER INIT-FORM ENV)))
                          (COND ((CONSP VAR/KV)
                          ...
                          (PUSH (NCONC (LIST VAR/KV) (AND INIT-FORM (LIST INIT-FORM))
                                       (AND SUPPLIED-P-PARAMETER (LIST SUPPLIED-P-PARAMETER)))
                                NEW-LAMBDA-LIST)))))))
        (:AUXVARS
         (ETYPECASE ARG
      ...
      (WHEN (OR (ATOM LAMBDA-LIST) (AND (CDR LAMBDA-LIST) (ATOM (CDR LAMBDA-LIST))))
...
(DEFUN WALK-SPECIALIZED-LAMBDA-LIST (WALKER LAMBDA-LIST DECLS ENV)
  (LET ((REQ-PARAMS
         (FLET ((AUGMENT-ENV (VAR)
           ...
           (LOOP UNTIL (OR (NULL LAMBDA-LIST) (MEMBER (CAR LAMBDA-LIST) LAMBDA-LIST-KEYWORDS)) AS VAR =
                 (WALK-VAR (POP LAMBDA-LIST)) DO (AUGMENT-ENV (IF (CONSP VAR) (CAR VAR) VAR)) COLLECT VAR))))
...
(DEFUN WALK-LAMBDA-EXPRESSION (WALKER FORM ENV &REST ARGS &AUX (LAMBDA-LIST (CADR FORM)) (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS DOC)
    ...
    (MULTIPLE-VALUE-BIND (LAMBDA-LIST ENV)
        (WALK-LAMBDA-LIST WALKER LAMBDA-LIST DECLS ENV)
      `(,(APPLY #'WALK-FUNCTION-NAME WALKER (CAR FORM) ENV ARGS) ,LAMBDA-LIST ,@(IF DOC `(,DOC))
        ,@(IF DECLS `((DECLARE ,@DECLS))) ,@(WALK-LIST WALKER FORMS ENV)))))
(DEFINE-SPECIAL-FORM-WALKER LAMBDA ((WALKER WALKER) FORM ENV) (WALK-LAMBDA-EXPRESSION WALKER FORM ENV))
(DEFUN WALK-VARIABLE-BINDING (WALKER P ENV &AUX (BINDING (IF (CONSP P) P (LIST P))))
  (LIST (WALK-ATOMIC-FORM WALKER :BINDING (CAR BINDING) ENV) (AND (CDR BINDING) (WALK-FORM WALKER (CADR BINDING) ENV))))
(DEFINE-SPECIAL-FORM-WALKER LET
  ((WALKER WALKER) FORM ENV &AUX (BINDINGS (MAPCAR (LAMBDA (P) (WALK-VARIABLE-BINDING WALKER P ENV)) (CADR FORM)))
   (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS
                   (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :VARIABLE (MAPCAR #'CAR BINDINGS) :DECLARE DECLS)))))
(DEFINE-SPECIAL-FORM-WALKER FLET
  ((WALKER WALKER) FORM ENV &AUX
   (BINDINGS (MAPCAR (LAMBDA (P) (WALK-LAMBDA-EXPRESSION WALKER P ENV :OPERATOR 'FLET :LOCAL T :DEF T)) (CADR FORM)))
   (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS
                   (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :FUNCTION (MAPCAR #'CAR BINDINGS) :DECLARE DECLS)))))
(DEFUN MAKE-MACRO-DEFINITIONS (WALKER DEFS ENV)
  (MAPCAR (LAMBDA (DEF &AUX (NAME (WALK-ATOMIC-FORM WALKER :SYMBOL-MACRO-BINDING (CAR DEF) ENV)))
            (LIST NAME (ENCLOSE (PARSE-MACRO NAME (CADR DEF) (CDDR DEF) ENV) ENV WALKER)))
          DEFS))
(DEFINE-SPECIAL-FORM-WALKER MACROLET
  ((WALKER WALKER) FORM ENV &AUX
   (BINDINGS
    (MAPCAR (LAMBDA (P) (WALK-LAMBDA-EXPRESSION WALKER P ENV :OPERATOR 'MACROLET :LOCAL T :DEF T)) (CADR FORM)))
   (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS
                   (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :MACRO (MAKE-MACRO-DEFINITIONS WALKER BINDINGS ENV) :DECLARE
                     DECLS)))))
(DEFINE-SPECIAL-FORM-WALKER SYMBOL-MACROLET
  ...
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :SYMBOL-MACRO BINDINGS :DECLARE DECLS)))))
(DEFINE-SPECIAL-FORM-WALKER LET* ((WALKER WALKER) FORM ENV &AUX (BINDINGS (CADR FORM)) (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM)
      ,(MAPCAR (LAMBDA (P &AUX (WALKED-BINDING (WALK-VARIABLE-BINDING WALKER P ENV))
                        (VARIABLE (LIST (CAR WALKED-BINDING))))
                 (SETQ ENV (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :VARIABLE VARIABLE :DECLARE DECLS))
                 WALKED-BINDING)
               BINDINGS)
      ,@(IF DECLS `((DECLARE ,@DECLS))) ,@(WALK-LIST WALKER FORMS ENV))))
(DEFINE-SPECIAL-FORM-WALKER LABELS ((WALKER WALKER) FORM ENV &AUX (BINDINGS (CADR FORM)) (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    (LET* ((FUNCTION-NAMES
            (MAPCAR (LAMBDA (P) (WALK-FUNCTION-NAME WALKER (CAR P) ENV :OPERATOR 'LABELS :LOCAL T :DEF T)) BINDINGS))
           (ENV (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :FUNCTION FUNCTION-NAMES :DECLARE DECLS)))
      `(,(CAR FORM) ,(MAPCAR (LAMBDA (P) (WALK-LAMBDA-EXPRESSION WALKER P ENV)) BINDINGS)
        ,@(IF DECLS `((DECLARE ,@DECLS))) ,@(WALK-LIST WALKER FORMS ENV)))))
(DEFINE-SPECIAL-FORM-WALKER LOCALLY ((WALKER WALKER) FORM ENV)
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY (CDR FORM) :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :DECLARE DECLS)))))
...
(DEFUN JOIN-STRINGS
    (STRINGS &OPTIONAL (DELIMITER #\Space) &AUX (STRINGS (ENSURE-LIST STRINGS)) (DELIMITER (STRING DELIMITER)))
  (WITH-OUTPUT-TO-STRING (OUT)
    (LOOP FOR (STRING . MORE) ON STRINGS DO (WRITE-STRING (STRING STRING) OUT) WHEN MORE DO
...
(DEFINE-METHOD-COMBINATION JOIN-STRINGS (&OPTIONAL (DELIMITER #\Space))
  ...
  (FLET ((CALL-METHODS (METHODS) (MAPCAR (LAMBDA (METHOD) `(ENSURE-LIST (CALL-METHOD ,METHOD))) METHODS)))
    `(JOIN-STRINGS (APPEND ,@(CALL-METHODS PREFIX) ,@(CALL-METHODS PRIMARY) ,@(CALL-METHODS (REVERSE SUFFIX)))
                   ,DELIMITER)))
...
(DEFCLASS TYPE-HEADING (HEADING)
          ((ALLOWABLE-MODIFIERS :READER ALLOWABLE-MODIFIERS :INITARG :ALLOWABLE-MODIFIERS :ALLOCATION :CLASS)
           (MODIFIERS :ACCESSOR HEADING-MODIFIERS :INITARG :MODIFIERS)))
(DEFMETHOD INITIALIZE-INSTANCE :AFTER ((HEADING TYPE-HEADING) &REST INITARGS &KEY &ALLOW-OTHER-KEYS)
  (SETF (HEADING-MODIFIERS HEADING)
        (LOOP WITH ALLOWABLE-MODIFIERS = (ALLOWABLE-MODIFIERS HEADING) FOR (NAME VALUE) ON INITARGS BY #'CDDR WHEN
              (AND (MEMBER NAME ALLOWABLE-MODIFIERS) VALUE) COLLECT NAME INTO MODIFIERS FINALLY
              (RETURN (SORT MODIFIERS
                            (LAMBDA (X Y) (< (POSITION X ALLOWABLE-MODIFIERS) (POSITION Y ALLOWABLE-MODIFIERS))))))))
(DEFMETHOD HEADING-NAME :PREFIX ((HEADING TYPE-HEADING)) (MAPCAR #'STRING-DOWNCASE (HEADING-MODIFIERS HEADING)))
(EVAL-WHEN (:COMPILE-TOPLEVEL :LOAD-TOPLEVEL :EXECUTE)
  (DEFUN TYPE-HEADING-CLASS-NAME (NAME) (INTERN (WITH-STANDARD-IO-SYNTAX (FORMAT NIL "~A-HEADING" NAME)))))
(DEFMACRO DEFINE-TYPE-HEADING (NAME SLOTS &REST OPTIONS)
  (FLET ((OPTION (INDICATOR) (CDR (FIND-IF (LAMBDA (OPTION) (EQ (CAR OPTION) INDICATOR)) OPTIONS))))
    `(DEFCLASS ,(TYPE-HEADING-CLASS-NAME NAME) (TYPE-HEADING) ,SLOTS
               (:DEFAULT-INITARGS :NAME ,(OR (CAR (OPTION :NAME)) (STRING-DOWNCASE NAME)) :ALLOWABLE-MODIFIERS
                ',(OPTION :MODIFIERS)))))
(DEFUN MAKE-TYPE-HEADING (TYPE &REST INITARGS) (APPLY #'MAKE-INSTANCE (TYPE-HEADING-CLASS-NAME TYPE) INITARGS))
(DEFINE-TYPE-HEADING FUNCTION NIL (:MODIFIERS :LOCAL :GENERIC :SETF))
(DEFINE-TYPE-HEADING METHOD ((QUALIFIERS :READER METHOD-HEADING-QUALIFIERS :INITARG :QUALIFIERS :INITFORM NIL))
                     (:MODIFIERS :SETF))
(DEFINE-TYPE-HEADING MACRO NIL (:MODIFIERS :LOCAL))
(DEFINE-TYPE-HEADING SYMBOL-MACRO NIL (:NAME "symbol macro") (:MODIFIERS :LOCAL))
...
(DEFMETHOD HEADING-NAME :PREFIX ((HEADING METHOD-HEADING))
  (MAPCAR #'STRING-DOWNCASE (OR (METHOD-HEADING-QUALIFIERS HEADING) '(:PRIMARY))))
...
(DEFUN GENERIC-FUNCTION-P (FUNCTION-NAME)
  (OR (TYPEP FUNCTION-NAME 'GENERIC-FUNCTION)
      (AND (FBOUNDP FUNCTION-NAME) (TYPEP (FDEFINITION FUNCTION-NAME) 'GENERIC-FUNCTION))))
...
(DEFMETHOD INDEX-VARIABLE ((INDEX INDEX) VARIABLE ENV SECTION)
  (MULTIPLE-VALUE-BIND (TYPE LOCAL)
    ...
    (WHEN (OR (MEMBER TYPE '(:SPECIAL :SYMBOL-MACRO :CONSTANT)) (AND *INDEX-LEXICAL-VARIABLES* (EQL TYPE :LEXICAL)))
      (ADD-INDEX-ENTRY INDEX
    ...
    (WHEN (MEMBER TYPE '(:FUNCTION :MACRO))
      (ADD-INDEX-ENTRY INDEX
                       (MAKE-HEADING FUNCTION-NAME
                                     (ECASE TYPE
                                       (:FUNCTION
                                        (MAKE-TYPE-HEADING 'FUNCTION :LOCAL LOCAL :GENERIC
...
(DEFMETHOD INDEX-DEFUN ((INDEX INDEX) FUNCTION-NAME SECTION &KEY OPERATOR LOCAL GENERIC QUALIFIERS)
  (ADD-INDEX-ENTRY INDEX
                   (MAKE-HEADING (ETYPECASE FUNCTION-NAME
                                   (SYMBOL FUNCTION-NAME)
                                   (SETF-FUNCTION-NAME (CADR FUNCTION-NAME)))
                                 (MAKE-TYPE-HEADING (CASE OPERATOR
                                                      ...
                                                      (OTHERWISE 'FUNCTION))
                                                    :SETF (SETF-FUNCTION-NAME-P FUNCTION-NAME) :LOCAL LOCAL :GENERIC
                                                    (OR GENERIC (GENERIC-FUNCTION-P FUNCTION-NAME)) :QUALIFIERS
                                                    QUALIFIERS))
                   SECTION
    :DEF T))
...
(DEFMETHOD INDEX-SYMBOL (INDEX CONTEXT SYMBOL ENV &KEY SECTION DEF)
  (DECLARE (IGNORE CONTEXT DEF)) (INDEX-VARIABLE INDEX SYMBOL ENV SECTION))
(DEFMACRO DEFINE-SYMBOL-INDEXER (CONTEXT TYPE &REST ARGS)
  `(DEFMETHOD INDEX-SYMBOL :AFTER (INDEX (CONTEXT (EQL ,CONTEXT)) SYMBOL ENV &KEY SECTION DEF)
     (DECLARE (IGNORE ENV))
     (ADD-INDEX-ENTRY INDEX (MAKE-HEADING SYMBOL (MAKE-TYPE-HEADING ,TYPE ,@ARGS)) SECTION :DEF DEF)))
(DEFMETHOD INDEX-SYMBOL :AFTER (INDEX (CONTEXT (EQL :BINDING)) SYMBOL ENV &KEY SECTION DEF)
  (DECLARE (IGNORE ENV))
  (WHEN *INDEX-LEXICAL-VARIABLES*
    (ADD-INDEX-ENTRY INDEX (MAKE-HEADING SYMBOL (MAKE-TYPE-HEADING 'VARIABLE)) SECTION :DEF DEF)))
...
(DEFMETHOD SECTION-CODE :AROUND ((SECTION SECTION))
  (LET ((CODE (CALL-NEXT-METHOD)))
    (IF *INDEXING* (SUBSTITUTE-SYMBOLS (TANGLE CODE :EXPAND-NAMED-SECTIONS NIL) SECTION) CODE)))
     ...
     (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
       ...
       (COND (SECTION
              (CASE (VARIABLE-INFORMATION SYMBOL ENV)
                (:SYMBOL-MACRO (INDEX-VARIABLE (WALKER-INDEX WALKER) SYMBOL ENV SECTION)
                 (CALL-NEXT-METHOD WALKER (CONS SYMBOL (CDR FORM)) ENV))
                (T FORM)))
             (T (CALL-NEXT-METHOD)))))
    (CONS (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
              (SYMBOL-PROVENANCE (CAR FORM))
            (COND (SECTION
                   (CASE (FUNCTION-INFORMATION SYMBOL ENV)
                     (:MACRO (INDEX-FUNCALL (WALKER-INDEX WALKER) SYMBOL FORM ENV SECTION)
                      (CALL-NEXT-METHOD WALKER (CONS SYMBOL (CDR FORM)) ENV))
                     (T FORM)))
                  (T (CALL-NEXT-METHOD)))))
    (T FORM)))
(DEFMETHOD WALK-ATOMIC-FORM :AROUND ((WALKER INDEXING-WALKER) CONTEXT FORM ENV &KEY DEF)
  (IF (SYMBOLP FORM)
      (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
          (SYMBOL-PROVENANCE FORM)
        (WHEN SECTION (INDEX-SYMBOL (WALKER-INDEX WALKER) CONTEXT SYMBOL ENV :SECTION SECTION :DEF DEF))
...
(DEFMETHOD WALK-COMPOUND-FORM :BEFORE ((WALKER INDEXING-WALKER) OPERATOR FORM ENV)
  (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
    ...
    (WHEN SECTION (INDEX-FUNCALL (WALKER-INDEX WALKER) SYMBOL FORM ENV SECTION))))
...
(MACROLET ((DEFINE-DEFUN-LIKE-WALKER (OPERATOR)
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR ((WALKER INDEXING-WALKER) FORM ENV)
                `(,(CAR FORM) ,@(WALK-LAMBDA-EXPRESSION WALKER (CDR FORM) ENV :OPERATOR (CAR FORM) :DEF T)))))
  (DEFINE-DEFUN-LIKE-WALKER DEFUN)
  (DEFINE-DEFUN-LIKE-WALKER DEFMACRO))
(MACROLET ((DEFINE-INDEXING-DEFVAR-WALKER (OPERATOR TYPE)
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR ((WALKER INDEXING-WALKER) FORM ENV)
                `(,(CAR FORM) ,(WALK-ATOMIC-FORM WALKER ,TYPE (CADR FORM) ENV :DEF T)
                  ,@(WALK-LIST WALKER (CDDR FORM) ENV)))))
...
(DEFMETHOD WALK-DECLARATION-SPECIFIERS ((WALKER INDEXING-WALKER) DECLS ENV)
  (LOOP FOR (IDENTIFIER . DATA) IN DECLS IF (EQL IDENTIFIER 'SPECIAL) COLLECT `(SPECIAL ,@(WALK-LIST WALKER DATA ENV))
        ELSE IF (EQL IDENTIFIER 'OPTIMIZE) COLLECT `(OPTIMIZE ,@DATA)))
...
(DEFINE-SPECIAL-FORM-WALKER DEFGENERIC ((WALKER INDEXING-WALKER) FORM ENV)
  (DESTRUCTURING-BIND (OPERATOR FUNCTION-NAME LAMBDA-LIST &REST OPTIONS)
      FORM
    `(,OPERATOR ,(WALK-FUNCTION-NAME WALKER FUNCTION-NAME ENV :OPERATOR 'DEFGENERIC :GENERIC T :DEF T)
      ,(WALK-LAMBDA-LIST WALKER LAMBDA-LIST NIL ENV)
      ,@(LOOP FOR FORM IN OPTIONS COLLECT
              (CASE (CAR FORM)
                (:METHOD-COMBINATION
                 `(,(CAR FORM) ,(WALK-ATOMIC-FORM WALKER :METHOD-COMBINATION (CADR FORM) ENV)
                   ,@(WALK-LIST WALKER (CDDR FORM) ENV)))
                (:METHOD
                 (LET* ((OPERATOR (POP FORM))
                        (QUALIFIERS
                         (MAPCAR (LAMBDA (Q) (WALK-ATOMIC-FORM WALKER :METHOD-QUALIFIER Q ENV)) (POP-QUALIFIERS FORM)))
...
(DEFUN WALK-METHOD-DEFINITION (WALKER OPERATOR FUNCTION-NAME QUALIFIERS LAMBDA-LIST BODY ENV)
  (MULTIPLE-VALUE-BIND (BODY-FORMS DECLS DOC)
    ...
    (MULTIPLE-VALUE-BIND (LAMBDA-LIST ENV)
      ...
      `(,OPERATOR ,@(WHEN FUNCTION-NAME `(,FUNCTION-NAME)) ,@QUALIFIERS ,LAMBDA-LIST ,@(IF DOC `(,DOC))
        ,@(IF DECLS `((DECLARE ,@DECLS))) ,@(WALK-LIST WALKER BODY-FORMS ENV)))))
(DEFINE-SPECIAL-FORM-WALKER DEFMETHOD
  ((WALKER INDEXING-WALKER) FORM ENV &AUX (OPERATOR (POP FORM)) (FUNCTION-NAME (POP FORM))
   (QUALIFIERS (MAPCAR (LAMBDA (Q) (WALK-ATOMIC-FORM WALKER :METHOD-QUALIFIER Q ENV)) (POP-QUALIFIERS FORM)))
   (LAMBDA-LIST (POP FORM)) (BODY FORM))
  (WALK-METHOD-DEFINITION WALKER OPERATOR
    (WALK-FUNCTION-NAME WALKER FUNCTION-NAME ENV :OPERATOR 'DEFMETHOD :GENERIC T :QUALIFIERS QUALIFIERS :DEF T)
             ...
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR ((WALKER INDEXING-WALKER) FORM ENV)
                (DESTRUCTURING-BIND (OPERATOR NAME SUPERS SLOT-SPECS &REST OPTIONS)
                  ...
                  `(,OPERATOR ,(WALK-ATOMIC-FORM WALKER ,TYPE NAME ENV :DEF T)
                    ,(MAPCAR (LAMBDA (SUPER) (WALK-ATOMIC-FORM WALKER ,TYPE SUPER ENV)) SUPERS)
                    ,(MAPCAR (LAMBDA (SPEC) (WALK-SLOT-SPECIFIER WALKER SPEC ENV)) SLOT-SPECS)
...
(DEFUN WALK-SLOT-SPECIFIER (WALKER SPEC ENV)
  (ETYPECASE SPEC
    ...
    (CONS (DESTRUCTURING-BIND (NAME &REST OPTIONS)
              SPEC
            (LOOP FOR (OPT-NAME OPT-VALUE) ON OPTIONS BY #'CDDR IF (MEMBER OPT-NAME '(:READER :WRITER :ACCESSOR)) DO
                  (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
            ...
            `(,(WALK-ATOMIC-FORM WALKER :SLOT-NAME NAME ENV) ,@(WALK-LIST WALKER OPTIONS ENV))))))
(DEFINE-SPECIAL-FORM-WALKER DEFINE-METHOD-COMBINATION ((WALKER INDEXING-WALKER) FORM ENV)
  `(,(CAR FORM) ,(WALK-ATOMIC-FORM WALKER :METHOD-COMBINATION (CADR FORM) ENV :DEF T)
    ,@(WALK-LIST WALKER (CDDR FORM) ENV)))
...
(DEFUN INDEX-SECTIONS (SECTIONS &KEY (INDEX *INDEX*) (WALKER (MAKE-INSTANCE 'INDEXING-WALKER :INDEX INDEX)))
  (LET ((*EVALUATING* T))
    ...
    (DOLIST (FORM (TANGLE-CODE-FOR-INDEXING SECTIONS) (WALKER-INDEX WALKER)) (WALK-FORM WALKER FORM))))
...
(SET-WEAVE-DISPATCH 'INDEX
                    (LAMBDA (STREAM INDEX)
                      (MAP-BST (LAMBDA (ENTRY) (WRITE ENTRY :STREAM STREAM)) (INDEX-ENTRIES INDEX))))
...
(DEFMETHOD PRINT-ENTRY-HEADING (STREAM (HEADING TYPE-HEADING) &KEY) (WRITE-STRING (HEADING-NAME HEADING) STREAM))
...
(DEFMETHOD ENTRY-HEADING-LESSP (H1 (H2 MACRO-CHAR-HEADING)) (DECLARE (IGNORE H1)))
...
(DEFMETHOD ENTRY-HEADING-EQUALP ((H1 MACRO-CHAR-HEADING) H2) (DECLARE (IGNORE H2)))
(DEFMETHOD ENTRY-HEADING-EQUALP (H1 (H2 MACRO-CHAR-HEADING)) (DECLARE (IGNORE H1)))
...
(DEFMETHOD INDEX-FUNCALL ((INDEX INDEX) (FUNCTION-NAME (EQL 'SET-MACRO-CHARACTER)) FORM ENV SECTION)
  (DECLARE (IGNORE ENV))
  (WHEN (CHARACTERP (SECOND FORM)) (ADD-INDEX-ENTRY INDEX (MAKE-MACRO-CHAR-HEADING (SECOND FORM)) SECTION :DEF T)))
(DEFMETHOD INDEX-FUNCALL ((INDEX INDEX) (FUNCTION-NAME (EQL 'SET-DISPATCH-MACRO-CHARACTER)) FORM ENV SECTION)
  (DECLARE (IGNORE ENV))
  (WHEN (CHARACTERP (SECOND FORM))
    (ADD-INDEX-ENTRY INDEX (MAKE-MACRO-CHAR-HEADING (SECOND FORM) (THIRD FORM)) SECTION :DEF T)))