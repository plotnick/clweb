...
(DEFMETHOD (SETF SECTION-CODE) (FORMS (SECTION NAMED-SECTION) &KEY (APPEND T))
           (SETF (SLOT-VALUE SECTION 'VALUE)
                   (IF (AND APPEND (SLOT-BOUNDP SECTION 'VALUE))
                       (APPEND (SLOT-VALUE SECTION 'VALUE) FORMS) FORMS)))
(DEFMETHOD FIND-OR-INSERT
...
(DEFUN DEFSECTION (NAME FORMS &OPTIONAL (APPEND-P T))
  (LET ((SECTION (FIND-SECTION NAME)))
    (SETF (SECTION-CODE SECTION :APPEND APPEND-P) FORMS)
    SECTION))
...
(DEFUN TANGLE (FORM)
  (LABELS ((EXPAND (FORM EXPANDED)
    ...
    (EXPAND FORM NIL)))
(DEFUN UNNAMED-SECTION-CODE (STREAM APPEND-P)
  (LOOP FOR SECTION IN (READ-SECTIONS STREAM) IF (SECTION-NAME SECTION) DO
        (DEFSECTION (SECTION-NAME SECTION) (SECTION-CODE SECTION) APPEND-P)
        ELSE APPEND (SECTION-CODE SECTION)))
(DEFUN LOAD-WEB-FROM-STREAM (STREAM VERBOSE PRINT &OPTIONAL (APPEND-P T))
  ...
  (LET ((*READTABLE* *READTABLE*) (*PACKAGE* *PACKAGE*))
    (DOLIST (FORM (TANGLE (UNNAMED-SECTION-CODE STREAM APPEND-P)) T)
      (IF PRINT
...
(DEFUN TANGLE-FILE
  ...
  (WITH-OPEN-FILE
    ...
    (WITH-OPEN-FILE
      ...
      (DOLIST (FORM (TANGLE (UNNAMED-SECTION-CODE INPUT T)))
        (PPRINT FORM LISP))))
