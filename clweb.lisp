...
(DEFMETHOD (SETF SECTION-CODE) (FORMS (SECTION NAMED-SECTION))
           (SETF (SLOT-VALUE SECTION 'VALUE)
                   (IF (SLOT-BOUNDP SECTION 'VALUE)
...
(DEFUN DEFSECTION (NAME FORMS)
  (LET ((SECTION (FIND-SECTION NAME)))
    (SETF (SECTION-CODE SECTION) FORMS)
    SECTION))
...
(DEFUN UNNAMED-SECTION-CODE (STREAM)
  (LOOP FOR SECTION IN (READ-SECTIONS STREAM) IF (SECTION-NAME SECTION) DO
        (DEFSECTION (SECTION-NAME SECTION) (SECTION-CODE SECTION)) ELSE APPEND
...
(DEFUN LOAD-WEB-FROM-STREAM (STREAM VERBOSE PRINT)
  ...
  (LET ((*READTABLE* *READTABLE*) (*PACKAGE* *PACKAGE*))
    (DOLIST (FORM (TANGLE (UNNAMED-SECTION-CODE STREAM)) T)
      (IF PRINT
...
(DEFUN LOAD-SECTIONS-FROM-TEMP-FILE (FILE &AUX (TRUENAME (PROBE-FILE FILE)))
  (WHEN TRUENAME
    (UNWIND-PROTECT
        (WITH-OPEN-FILE (STREAM TRUENAME :DIRECTION :INPUT)
          (LOAD-WEB-FROM-STREAM STREAM NIL T))
