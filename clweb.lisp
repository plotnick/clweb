...
(DEFCLASS HEADING NIL ((NAME :READER HEADING-NAME :INITARG :NAME)))
      ...
      (ADD-INDEX-ENTRY INDEX
                       (LIST VARIABLE
                             (ECASE TYPE
                               ...
                               (:SPECIAL
                                (MAKE-TYPE-HEADING 'VARIABLE :SPECIAL T))
                               (:CONSTANT
                                (MAKE-TYPE-HEADING 'VARIABLE :CONSTANT T))
                               (:SYMBOL-MACRO
    ...
    (WHEN (MEMBER TYPE '(:FUNCTION :MACRO))
      (ADD-INDEX-ENTRY INDEX
                       (LIST FUNCTION-NAME
                             (ECASE TYPE
                               (:FUNCTION
                                (MAKE-TYPE-HEADING 'FUNCTION :LOCAL LOCAL
                                                   :GENERIC
                                                   (GENERIC-FUNCTION-P
                                                    FUNCTION-NAME)))
                               (:MACRO
...
(DEFMACRO DEFINE-SYMBOL-INDEXER (CONTEXT TYPE &REST ARGS)
  `(DEFMETHOD INDEX-SYMBOL :AFTER
     ...
     (DECLARE (IGNORE ENV))
     (ADD-INDEX-ENTRY INDEX (LIST SYMBOL (MAKE-TYPE-HEADING ,TYPE ,@ARGS))
                      SECTION :DEF DEF)))
(DEFMETHOD INDEX-SYMBOL :AFTER
           (INDEX (CONTEXT (EQL :BINDING)) SYMBOL ENV &KEY SECTION DEF)
  (DECLARE (IGNORE ENV))
  (WHEN *INDEX-LEXICAL-VARIABLES*
    (ADD-INDEX-ENTRY INDEX (LIST SYMBOL (MAKE-TYPE-HEADING 'VARIABLE)) SECTION
                     :DEF DEF)))
