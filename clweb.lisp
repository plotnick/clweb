...
(DEFTEST READ-LIST (MARKER-VALUE (READ-FORM-FROM-STRING "(a b c)")) (A B C))
(DEFTEST READ-DOTTED-LIST (MARKER-VALUE (READ-FORM-FROM-STRING "(a b . c)"))
         (A B . C))
(DEFTEST LIST-MARKER-CHARPOS
         (LIST-MARKER-CHARPOS (READ-FORM-FROM-STRING "(a b c)")) (1 3 5))
...
(DEFTEST READ-BACKQUOTE (MARKER-VALUE (READ-FORM-FROM-STRING "`(a b c)"))
         '(A B C))
...
(DOLIST (MODE '(:LISP :INNER-LISP))
  (SET-DISPATCH-MACRO-CHARACTER #\# #\( #'SIMPLE-VECTOR-READER
                                (READTABLE-FOR-MODE MODE)))
(DEFTEST READ-SIMPLE-VECTOR (MARKER-VALUE (READ-FORM-FROM-STRING "#5(a b c)"))
         #(A B C C C))
...
(DEFUN START-CODE-READER (STREAM SUB-CHAR ARG)
  (DECLARE (IGNORE STREAM ARG))
  (MAKE-INSTANCE 'START-CODE-MARKER :EVALP (CHAR= (CHAR-DOWNCASE SUB-CHAR) #\e)
