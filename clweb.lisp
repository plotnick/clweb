...
(DEFCLASS TEST-SECTION (SECTION) ((TEST-FOR :ACCESSOR TEST-FOR-SECTION)))
...
(DEFGENERIC MAP-BST
    #'TREE)
(DEFMETHOD MAP-BST #'(TREE (EQL NIL)) (DECLARE (IGNORE FUNCTION)))
           ...
           (WITH-SLOTS (CHARPOS)
             ...
             (LOOP FOR CHAR ACROSS (GET-CHARPOS-STREAM-BUFFER STREAM)
                   DO (CASE CHAR
                        (#\Tab
                         (INCF CHARPOS
                               (- *TAB-WIDTH* (REM CHARPOS *TAB-WIDTH*))))
...
(DEFUN WEAVE
  ...
  (WHEN (AND TESTS-FILE (> (LENGTH *TEST-SECTIONS*) 1))
    ...
    (WEAVE-SECTIONS *TEST-SECTIONS* :OUTPUT-FILE TESTS-FILE :PRINT PRINT
                    :VERBOSE VERBOSE :EXTERNAL-FORMAT EXTERNAL-FORMAT))
  (WHEN VERBOSE (FORMAT T "~&; weaving sections to ~A~%" OUTPUT-FILE))
  (WEAVE-SECTIONS *SECTIONS* :OUTPUT-FILE OUTPUT-FILE :INDEX-FILE INDEX-FILE
                  :SECTIONS-FILE SECTIONS-FILE :VERBOSE VERBOSE :PRINT PRINT
                  :EXTERNAL-FORMAT EXTERNAL-FORMAT))
(DEFUN WEAVE-SECTIONS
       (SECTIONS
        &KEY OUTPUT-FILE INDEX-FILE SECTIONS-FILE (VERBOSE *WEAVE-VERBOSE*)
...
(DEFCLASS FUNCTION-HEADING (GLOBAL/LOCAL-HEADING) ((GENERIC :INITARG :GENERIC))
...
(DEFMETHOD WALK-COMPOUND-FORM :BEFORE
           ((WALKER INDEXING-WALKER) OPERATOR FORM ENV) (DECLARE (IGNORE FORM))
           (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
