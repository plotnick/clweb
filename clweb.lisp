...
(DEFUN TANGLE (FORM &KEY (EXPAND-NAMED-SECTIONS-P T))
  (LABELS ((EXPAND (FORM EXPANDED)
             (MULTIPLE-VALUE-BIND (NEW-FORM NEWLY-EXPANDED-P)
                 (TANGLE-1 FORM :EXPAND-NAMED-SECTIONS-P
                           EXPAND-NAMED-SECTIONS-P)
               (IF NEWLY-EXPANDED-P
                   (EXPAND NEW-FORM T)
...
(DEFUN WALK-LAMBDA-EXPRESSION
  ...
  (MULTIPLE-VALUE-BIND (FORMS DECLS DOC)
    ...
    (MULTIPLE-VALUE-BIND (LAMBDA-LIST ENV)
      ...
      `(,(WALK-FUNCTION-NAME WALKER (CAR FORM) ENV) ,LAMBDA-LIST
        ,@(IF DOC
              `(,DOC))
        ,@(IF DECLS
              `((DECLARE ,@DECLS)))
...
(DEFINE-SPECIAL-FORM-WALKER FLET
    ((WALKER WALKER) FORM ENV &AUX
     (BINDINGS
      (MAPCAR (LAMBDA (P) (WALK-LAMBDA-EXPRESSION WALKER P ENV)) (CADR FORM)))
     (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,BINDINGS
...
(DEFINE-SPECIAL-FORM-WALKER LABELS
  ...
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    (LET* ((FUNCTION-NAMES
            (MAPCAR (LAMBDA (P) (WALK-FUNCTION-NAME WALKER (CAR P) ENV))
                    BINDINGS))
           (ENV
...
(DEFMETHOD SECTION-CODE :AROUND ((SECTION SECTION))
           (LET ((CODE (CALL-NEXT-METHOD)))
             (IF *INDEXING*
                 (SUBSTITUTE-SYMBOLS (TANGLE CODE :EXPAND-NAMED-SECTIONS-P NIL)
                                     SECTION)
                 CODE)))
