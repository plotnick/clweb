...
(DEFCLASS COMMA NIL ((FORM :INITARG :FORM)))
...
(DEFUN PRINT-SECTION (STREAM SECTION)
  (FORMAT STREAM "~&\\~:[M~*~;N{~D}~]{~D}" (STARRED-SECTION-P SECTION)
          (SECTION-DEPTH SECTION) (SECTION-NUMBER SECTION))
  (LET* ((COMMENTARY (SECTION-COMMENTARY SECTION))
...
(DEFCLASS WALKER NIL
          ((GLOBAL-ENVIRONMENT :ACCESSOR WALKER-GLOBAL-ENVIRONMENT :INITFORM
...
(DEFGENERIC WALKER-VARIABLE-INFORMATION
    (WALKER SYMBOL &OPTIONAL ENV))
(DEFGENERIC WALKER-FUNCTION-INFORMATION
    (WALKER FUNCTION &OPTIONAL ENV))
(DEFGENERIC WALKER-DECLARATION-INFORMATION
    (WALKER DECL &OPTIONAL ENV))
...
(DEFMETHOD WALKER-VARIABLE-INFORMATION ((WALKER WALKER) SYMBOL &OPTIONAL ENV)
  (MULTIPLE-VALUE-BIND (TYPE LOCAL DECLS)
      (VARIABLE-INFORMATION SYMBOL (OR ENV (WALKER-GLOBAL-ENVIRONMENT WALKER)))
    (VALUES TYPE (AND ENV LOCAL) DECLS)))
(DEFMETHOD WALKER-FUNCTION-INFORMATION ((WALKER WALKER) FUNCTION &OPTIONAL ENV)
  (FUNCTION-INFORMATION FUNCTION (OR ENV (WALKER-GLOBAL-ENVIRONMENT WALKER))))
...
(DEFMETHOD WALK-FORM
           ((WALKER WALKER) FORM
            &OPTIONAL (ENV (ENSURE-PORTABLE-WALKING-ENVIRONMENT NIL))
            &AUX (EXPANDED T))
  (LOOP
   (CATCH 'CONTINUE-WALK
     (COND
      ...
      ((AND (SYMBOLP FORM)
       ...
       (MULTIPLE-VALUE-SETQ (FORM EXPANDED)
         (MACROEXPAND-FOR-WALK WALKER
                               (WALK-ATOMIC-FORM WALKER :SYMBOL-MACRO FORM NIL)
                               NIL))
...
(DEFINE-SPECIAL-FORM-WALKER THE
    ((WALKER WALKER) FORM ENV)
...
(DEFINE-SPECIAL-FORM-WALKER EVAL-WHEN
    ((WALKER WALKER) FORM ENV)
  `(,(CAR FORM) ,(WALK-LIST WALKER (CADR FORM) ENV)
    ,@(WALK-LIST WALKER (CDDR FORM) ENV)))
...
(DEFINE-SPECIAL-FORM-WALKER DEFINE-SYMBOL-MACRO
    ((WALKER WALKER) FORM ENV)
  (LET ((SYMBOL
         (WALK-ATOMIC-FORM WALKER :DEFINE-SYMBOL-MACRO (SECOND FORM) ENV)))
    (SETF (WALKER-GLOBAL-ENVIRONMENT WALKER)
            (AUGMENT-WALKER-ENVIRONMENT WALKER
                                        (WALKER-GLOBAL-ENVIRONMENT WALKER)
                                        :SYMBOL-MACRO
                                        `((,SYMBOL ,(THIRD FORM)))))
    (THROW 'CONTINUE-WALK FORM)))
...
(DEFGENERIC COMPUTE-TYPE-HEADING-CLASS
    (OPERATOR &KEY &ALLOW-OTHER-KEYS))
...
(DEFINE-TYPE-HEADING METHOD
                     ...
                     (:HEADING-NAME :PREFIX (HEADING)
                      (MAPCAR #'STRING-DOWNCASE
                              (OR (METHOD-HEADING-QUALIFIERS HEADING)
                                  '(:PRIMARY))))
                     (:MODIFIERS :SETF))
(DEFINE-TYPE-HEADING MACRO NIL (:MODIFIERS :LOCAL)
                     (:OPERATORS DEFMACRO MACROLET))
(DEFINE-TYPE-HEADING SYMBOL-MACRO NIL (:MODIFIERS :LOCAL)
                     (:OPERATORS DEFINE-SYMBOL-MACRO SYMBOL-MACROLET))
(DEFINE-TYPE-HEADING CLASS NIL (:OPERATORS DEFCLASS))
(DEFINE-TYPE-HEADING CONDITION-CLASS NIL (:OPERATORS DEFINE-CONDITION))
(DEFINE-TYPE-HEADING STRUCTURE NIL (:OPERATORS DEFSTRUCT))
(DEFINE-TYPE-HEADING VARIABLE NIL (:MODIFIERS :SPECIAL :CONSTANT))
(DEFINE-TYPE-HEADING METHOD-COMBINATION NIL
                     (:OPERATORS DEFINE-METHOD-COMBINATION))
(DEFINE-TYPE-HEADING COMPILER-MACRO NIL (:OPERATORS DEFINE-COMPILER-MACRO))
(DEFMETHOD COMPUTE-TYPE-HEADING-CLASS
           ((OPERATOR (EQL (QUOTE DEFMETHOD))) &KEY QUALIFIERS FUNCTION-NAME)
...
(DEFMETHOD INDEX-DEFUN
  ...
  (ADD-INDEX-ENTRY INDEX
                   (MAKE-HEADING
                    ...
                    (MAKE-TYPE-HEADING
                     (COMPUTE-TYPE-HEADING-CLASS OPERATOR :QUALIFIERS
    ...
    (SYMBOL
     (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
       ...
       (IF SECTION
           (LET ((LOCAL))
             (CASE
                 (OR (WALKER-VARIABLE-INFORMATION WALKER SYMBOL ENV)
...
(DEFMETHOD WALK-ATOMIC-FORM :AROUND
  ...
  (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
      (SYMBOL-PROVENANCE FORM)
    (WHEN SECTION
      (INDEX-SYMBOL (WALKER-INDEX WALKER) CONTEXT SYMBOL ENV :SECTION SECTION
                    :DEF DEF))
    (CALL-NEXT-METHOD WALKER CONTEXT SYMBOL ENV)))
(DEFMETHOD WALK-ATOMIC-FORM :AROUND
           ((WALKER INDEXING-WALKER) (CONTEXT (EQL :BINDING)) (FORM SYMBOL) ENV
            &KEY)
...
(DEFMETHOD WALK-ATOMIC-FORM :AROUND
           ((WALKER INDEXING-WALKER) (CONTEXT (EQL :DEFINE-SYMBOL-MACRO))
            (FORM SYMBOL) ENV
            &KEY)
  (CALL-NEXT-METHOD WALKER CONTEXT FORM ENV :DEF T))
...
(DEFMETHOD INDEX-FUNCALL
  ...
  (DECLARE (IGNORE ENV))
  (WHEN (CHARACTERP (SECOND FORM))
    (ADD-INDEX-ENTRY INDEX (MAKE-MACRO-CHAR-HEADING (SECOND FORM) (THIRD FORM))
                     SECTION :DEF T)))