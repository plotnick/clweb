...
(DEFUN TESTS-FILE-PATHNAME
  ...
  (IF TESTS-FILE
      ...
      (UNLESS TESTS-FILE-SUPPLIED
        (MERGE-PATHNAMES
         (MAKE-PATHNAME :HOST (PATHNAME-HOST OUTPUT-FILE :CASE :COMMON) :NAME
                        (CONCATENATE 'STRING
                                     (PATHNAME-NAME INPUT-FILE :CASE :COMMON)
                                     "-TESTS")
                        :TYPE TYPE :CASE :COMMON)
         OUTPUT-FILE))))
(DEFUN MAKE-TYPE-PATHNAME
       (TYPE &OPTIONAL (DEFAULTS *DEFAULT-PATHNAME-DEFAULTS*))
  (MERGE-PATHNAMES
   (MAKE-PATHNAME :HOST (PATHNAME-HOST DEFAULTS :CASE :COMMON) :TYPE TYPE :CASE
                  :COMMON)
   DEFAULTS))
(DEFUN TANGLE-FILE
       (INPUT-FILE
...
(DEFUN WEAVE-PATHNAMES
  ...
  (LET* ((INPUT-FILE
          (MERGE-PATHNAMES INPUT-FILE
                           (MAKE-PATHNAME :TYPE "CLW" :CASE :COMMON)))
         (OUTPUT-FILE
          (LET ((OUTPUT-FILE-DEFAULTS (MAKE-TYPE-PATHNAME "TEX" INPUT-FILE)))
            (IF OUTPUT-FILE
                (MERGE-PATHNAMES OUTPUT-FILE OUTPUT-FILE-DEFAULTS)
                OUTPUT-FILE-DEFAULTS)))
         (INDEX-FILE
          (LET ((INDEX-FILE-DEFAULTS (MAKE-TYPE-PATHNAME "IDX" OUTPUT-FILE)))
            (IF INDEX-FILE
                (MERGE-PATHNAMES INDEX-FILE INDEX-FILE-DEFAULTS)
                (WHEN (NOT INDEX-FILE-SUPPLIED) INDEX-FILE-DEFAULTS))))
         (SECTIONS-FILE
          (WHEN INDEX-FILE (MAKE-TYPE-PATHNAME "SCN" INDEX-FILE))))
