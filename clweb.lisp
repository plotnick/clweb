...
(DEFVAR *TESTS-FILE-PATHNAME-FUNCTION*
  (LAMBDA (INPUT-FILE OUTPUT-FILE)
    ...
    (MAKE-PATHNAME :NAME
                   (CONCATENATE 'STRING (PATHNAME-NAME OUTPUT-FILE) "-tests")
                   :TYPE (PATHNAME-TYPE OUTPUT-FILE) :VERSION :NEWEST :DEFAULTS
                   OUTPUT-FILE)))
...
(DEFUN LISP-FILE-PATHNAME (FILENAME)
  (MAKE-PATHNAME :TYPE (PATHNAME-TYPE *LISP-SOURCE-DEFAULTS*) :VERSION :NEWEST
                 :DEFAULTS FILENAME))
...
(DEFUN TESTS-FILE-PATHNAME
  ...
  (ASSERT (OR OUTPUT-FILE TESTS-FILE-SUPPLIED) (OUTPUT-FILE TESTS-FILE)
          "Either an output or a tests filename must be supplied.")
  (IF TESTS-FILE
      (MAKE-PATHNAME :TYPE (PATHNAME-TYPE OUTPUT-FILE) :VERSION :NEWEST
                     :DEFAULTS TESTS-FILE)
...
(DEFUN TANGLE-FILE-PATHNAMES
  ...
  (LET* ((INPUT-FILE (INPUT-FILE-PATHNAME INPUT-FILE))
         ...
         (TESTS-OUTPUT-FILE
          (AND TESTS-FILE
               (MAKE-PATHNAME :TYPE (PATHNAME-TYPE OUTPUT-FILE) :VERSION
                              :NEWEST :DEFAULTS TESTS-FILE))))
...
(DEFUN WEAVE-PATHNAMES
  ...
  (LET* ((INPUT-FILE (INPUT-FILE-PATHNAME INPUT-FILE))
         (OUTPUT-FILE
          (OR OUTPUT-FILE
              (MAKE-PATHNAME :TYPE (PATHNAME-TYPE *TEX-SOURCE-DEFAULTS*)
                             :VERSION :NEWEST :DEFAULTS INPUT-FILE)))
         (INDEX-FILE
          (OR INDEX-FILE
              (UNLESS INDEX-FILE-SUPPLIED
                (MAKE-PATHNAME :TYPE (PATHNAME-TYPE *INDEX-FILE-DEFAULTS*)
                               :VERSION :NEWEST :DEFAULTS OUTPUT-FILE))))
         (SECTIONS-FILE
          (AND INDEX-FILE
               (MAKE-PATHNAME :TYPE (PATHNAME-TYPE *SECTIONS-FILE-DEFAULTS*)
                              :VERSION :NEWEST :DEFAULTS INDEX-FILE))))
    (VALUES OUTPUT-FILE INDEX-FILE SECTIONS-FILE)))
(DEFCLASS CLWEB-FILE (ASDF/COMPONENT:SOURCE-FILE) ((TYPE :INITFORM "clw")))
(DEFMETHOD ASDF/COMPONENT:COMPONENT-PATHNAME ((COMPONENT CLWEB-FILE))
  (INPUT-FILE-PATHNAME (CALL-NEXT-METHOD)))
(DEFMETHOD ASDF/ACTION:OUTPUT-FILES
           ((OP ASDF/LISP-ACTION:COMPILE-OP) (COMPONENT CLWEB-FILE))
  (VALUES
   (MULTIPLE-VALUE-LIST
    (TANGLE-FILE-PATHNAMES (ASDF/COMPONENT:COMPONENT-PATHNAME COMPONENT)))
   NIL))
(DEFMETHOD ASDF/ACTION:PERFORM
           ((OP ASDF/LISP-ACTION:COMPILE-OP) (COMPONENT CLWEB-FILE))
  (TANGLE-FILE (ASDF/COMPONENT:COMPONENT-PATHNAME COMPONENT) :OUTPUT-FILE
               (FIRST (ASDF/ACTION:OUTPUT-FILES OP COMPONENT))))
(DEFMETHOD ASDF/ACTION:PERFORM
           ((OP ASDF/LISP-ACTION:LOAD-OP) (COMPONENT CLWEB-FILE))
  (MAP NIL #'LOAD
       (REMOVE-IF (LAMBDA (FILE) (STRING= (PATHNAME-TYPE FILE) "lisp"))
                  (ASDF/ACTION:INPUT-FILES OP COMPONENT))))
(DEFMETHOD ASDF/ACTION:PERFORM
           ((OP ASDF/LISP-ACTION:LOAD-SOURCE-OP) (COMPONENT CLWEB-FILE))
  (LOAD-WEB (ASDF/COMPONENT:COMPONENT-PATHNAME COMPONENT)))
(DEFCLASS WEAVE-OP (ASDF/OPERATION:OPERATION) NIL)
(DEFMETHOD ASDF/ACTION:OUTPUT-FILES ((OP WEAVE-OP) (COMPONENT CLWEB-FILE))
  (VALUES
   (MULTIPLE-VALUE-LIST
    (WEAVE-PATHNAMES (ASDF/COMPONENT:COMPONENT-PATHNAME COMPONENT)))
   T))
(DEFMETHOD ASDF/ACTION:PERFORM ((OP WEAVE-OP) COMPONENT)
  (DECLARE (IGNORE COMPONENT)))
(DEFMETHOD ASDF/ACTION:PERFORM ((OP WEAVE-OP) (COMPONENT CLWEB-FILE))
  (WEAVE (ASDF/COMPONENT:COMPONENT-PATHNAME COMPONENT)))
     ...
     (LET* ((*READTABLE* READTABLE)
       ...
       (WITH-COMPILATION-UNIT NIL
         (MULTIPLE-VALUE-PROG1
             (COMPILE-FILE LISP-FILE :OUTPUT-FILE OUTPUT-FILE :VERBOSE VERBOSE
