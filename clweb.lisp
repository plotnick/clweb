...
(DEFPACKAGE "CLWEB" (:USE "COMMON-LISP" #+:SBCL "SB-CLTL2" #+:ALLEGRO "SYS")
            (:EXPORT "TANGLE-FILE" "LOAD-WEB" "WEAVE" "LOAD-SECTIONS-FROM-TEMP-FILE" "AMBIGUOUS-PREFIX-ERROR"
...
(DEFINE-CONDITION INVALID-FUNCTION-NAME (PARSE-ERROR) ((NAME :INITARG :NAME :READER FUNCTION-NAME))
                  (:REPORT
                   (LAMBDA (ERROR STREAM)
...
(DEFCLASS TEST-SECTION (SECTION) ((TEST-FOR :ACCESSOR TEST-FOR-SECTION :INITFORM *CURRENT-SECTION*)))
...
(DEFMETHOD INITIALIZE-INSTANCE :AFTER ((SECTION SECTION) &REST INITARGS &KEY)
  (DECLARE (IGNORE INITARGS)) (SETQ *CURRENT-SECTION* (PUSH-SECTION SECTION)))
...
(DEFMETHOD FIND-OR-INSERT (ITEM (ROOT BINARY-SEARCH-TREE) &KEY (PREDICATE #'<) (TEST #'EQL)
...
(DEFUN MAPAPPEND (FUN &REST ARGS)
  (IF (SOME #'NULL ARGS)
      NIL
    (APPEND (APPLY FUN (MAPCAR #'CAR ARGS)) (APPLY #'MAPAPPEND FUN (MAPCAR #'CDR ARGS)))))
...
(DEFMETHOD FIND-OR-INSERT (ITEM (ROOT NAMED-SECTION) &KEY (PREDICATE #'SECTION-NAME-LESSP)
        ...
        (OR (DOLIST (CHILD (LIST (LEFT-CHILD NODE) (RIGHT-CHILD NODE)))
              (WHEN CHILD
                (MULTIPLE-VALUE-BIND (ALT PRESENT-P)
                  ...
                  (WHEN PRESENT-P
                    (RESTART-CASE (ERROR 'AMBIGUOUS-PREFIX-ERROR :PREFIX ITEM :FIRST-MATCH (NODE-KEY NODE)
...
(DEFUN SQUEEZE (STRING)
  (LOOP WITH SQUEEZING = NIL FOR CHAR ACROSS (STRING-TRIM *WHITESPACE* STRING) IF (NOT SQUEEZING) IF
        (WHITESPACEP CHAR) DO (SETQ SQUEEZING T) AND COLLECT #\Space INTO CHARS ELSE COLLECT CHAR INTO CHARS
        ELSE UNLESS (WHITESPACEP CHAR) DO (SETQ SQUEEZING NIL) AND COLLECT CHAR INTO CHARS FINALLY
...
(DEFCLASS CHARPOS-STREAM NIL
          ((CHARPOS :INITARG :CHARPOS) (PROXY-STREAM :ACCESSOR CHARPOS-PROXY-STREAM :INITARG :PROXY))
  ...
  (APPLY #'CALL-NEXT-METHOD INSTANCE SLOT-NAMES
         (LIST* :PROXY
                (MAKE-ECHO-STREAM STREAM
                                  (MAKE-STRING-OUTPUT-STREAM :ELEMENT-TYPE (STREAM-ELEMENT-TYPE STREAM)))
                INITARGS)))
...
(DEFMETHOD SHARED-INITIALIZE :AROUND ((INSTANCE CHARPOS-OUTPUT-STREAM) SLOT-NAMES &REST INITARGS &KEY STREAM)
  (APPLY #'CALL-NEXT-METHOD INSTANCE SLOT-NAMES
         (LIST* :PROXY
                (MAKE-BROADCAST-STREAM (MAKE-STRING-OUTPUT-STREAM :ELEMENT-TYPE (STREAM-ELEMENT-TYPE STREAM))
                  STREAM)
                INITARGS)))
...
(DEFMETHOD INITIALIZE-INSTANCE :AFTER ((INSTANCE CHARPOS-STREAM) &REST INITARGS &KEY)
  (DECLARE (IGNORE INITARGS)) (SETF (GETHASH (CHARPOS-PROXY-STREAM INSTANCE) *CHARPOS-STREAMS*) INSTANCE))
...
(DEFUN MAKE-CHARPOS-INPUT-STREAM (INPUT-STREAM &KEY (CHARPOS 0))
  (MAKE-INSTANCE 'CHARPOS-INPUT-STREAM :STREAM
                 (CASE INPUT-STREAM ((T) *TERMINAL-IO*) ((NIL) *STANDARD-INPUT*) (OTHERWISE INPUT-STREAM))
                 :CHARPOS CHARPOS))
(DEFUN MAKE-CHARPOS-OUTPUT-STREAM (OUTPUT-STREAM &KEY (CHARPOS 0))
  (MAKE-INSTANCE 'CHARPOS-OUTPUT-STREAM :STREAM
                 (CASE OUTPUT-STREAM ((T) *TERMINAL-IO*) ((NIL) *STANDARD-OUTPUT*) (OTHERWISE OUTPUT-STREAM))
                 :CHARPOS CHARPOS))
...
(DEFMACRO WITH-CHARPOS-OUTPUT-STREAM ((VAR STREAM &KEY (CHARPOS 0)) &BODY BODY)
  `(LET ((,VAR (CHARPOS-PROXY-STREAM (MAKE-CHARPOS-OUTPUT-STREAM ,STREAM :CHARPOS ,CHARPOS))))
     (UNWIND-PROTECT (PROGN ,@BODY) (RELEASE-CHARPOS-STREAM ,VAR))))
(DEFMACRO WITH-REWIND-STREAM ((VAR STREAM &OPTIONAL (REWIND 'REWIND)) &BODY BODY &AUX (OUT (GENSYM)))
  `(WITH-OPEN-STREAM (,OUT (MAKE-STRING-OUTPUT-STREAM))
     (WITH-OPEN-STREAM (,VAR (MAKE-ECHO-STREAM ,STREAM ,OUT))
       (FLET ((,REWIND NIL
                (SETQ ,VAR
                      (MAKE-CONCATENATED-STREAM (MAKE-STRING-INPUT-STREAM (GET-OUTPUT-STREAM-STRING ,OUT))
                        ,VAR))))
         ,@BODY))))
(DEFMACRO READ-WITH-ECHO
    ((STREAM VALUES ECHOED &KEY PREFIX) &BODY BODY &AUX (OUT (GENSYM)) (ECHO (GENSYM)) (REWIND (GENSYM))
     (RAW-OUTPUT (GENSYM)) (LENGTH (GENSYM)))
  `(WITH-OPEN-STREAM (,OUT (MAKE-STRING-OUTPUT-STREAM))
     (WITH-OPEN-STREAM (,ECHO (MAKE-ECHO-STREAM ,STREAM ,OUT))
       (WITH-OPEN-STREAM (,REWIND
                          (MAKE-CONCATENATED-STREAM ,@(WHEN PREFIX `((MAKE-STRING-INPUT-STREAM ,PREFIX)))
                            ,ECHO))
         (LET* ((,VALUES (MULTIPLE-VALUE-LIST (READ-PRESERVING-WHITESPACE ,REWIND)))
...
(SET-MACRO-CHARACTER #\Newline
                     (LAMBDA (STREAM CHAR) (DECLARE (IGNORE STREAM CHAR)) (MAKE-INSTANCE 'NEWLINE-MARKER))
...
(DEFUN LIST-READER (STREAM CHAR)
  (DECLARE (IGNORE CHAR))
  (LOOP WITH LIST = 'NIL WITH CHARPOS-LIST = 'NIL FOR N FROM 0 FOR FIRST-CHAR = (PEEK-CHAR T STREAM T NIL T)
        AS CHARPOS = (STREAM-CHARPOS STREAM) UNTIL (CHAR= FIRST-CHAR #\)) IF (CHAR= FIRST-CHAR #\.) DO
        (WITH-REWIND-STREAM (STREAM STREAM)
          ...
          (LET ((NEXT-CHAR (READ-CHAR STREAM T)))
            (COND ((TOKEN-DELIMITER-P NEXT-CHAR)
                  ...
                  (T (REWIND) (PUSH (READ STREAM T NIL T) LIST) (PUSH CHARPOS CHARPOS-LIST)))))
        ELSE DO (PUSH (READ STREAM T NIL T) LIST) (PUSH CHARPOS CHARPOS-LIST) FINALLY
        (READ-CHAR STREAM T NIL T)
        (RETURN (MAKE-INSTANCE 'LIST-MARKER :LENGTH N :LIST (NREVERSE LIST) :CHARPOS
...
(DEFMETHOD MARKER-VALUE ((MARKER BACKQUOTE-MARKER))
  (LET ((*PRINT-PRETTY* NIL) (*PRINT-READABLY* T) (*PRINT-MARKER* T) (*READTABLE* (READTABLE-FOR-MODE NIL)))
...
(DEFCLASS COMMA-MARKER (MARKER)
          ((FORM :READER COMMA-FORM :INITARG :FORM) (MODIFIER :READER COMMA-MODIFIER :INITARG :MODIFIER))
...
(DEFUN COMMA-READER (STREAM CHAR)
  ...
  (CASE (PEEK-CHAR NIL STREAM T NIL T)
    ((#\@ #\.) (MAKE-INSTANCE 'COMMA-MARKER :MODIFIER (READ-CHAR STREAM) :FORM (READ STREAM T NIL T)))
    (T (MAKE-INSTANCE 'COMMA-MARKER :FORM (READ STREAM T NIL T)))))
(DOLIST (MODE '(:LISP :INNER-LISP)) (SET-MACRO-CHARACTER #\, #'COMMA-READER NIL (READTABLE-FOR-MODE MODE)))
...
(DEFUN RADIX-READER (STREAM SUB-CHAR ARG)
  (MAKE-INSTANCE 'RADIX-MARKER :BASE (OR (CDR (ASSOC (CHAR-UPCASE SUB-CHAR) *RADIX-PREFIX-ALIST*)) ARG)
                 :VALUE
                 (FUNCALL (GET-DISPATCH-MACRO-CHARACTER #\# SUB-CHAR (READTABLE-FOR-MODE NIL)) STREAM
                          SUB-CHAR ARG)))
(DOLIST (MODE '(:LISP :INNER-LISP))
...
(DEFUN SNARF-UNTIL-CONTROL-CHAR
    (STREAM CONTROL-CHARS &AUX (CONTROL-CHARS (IF (LISTP CONTROL-CHARS) CONTROL-CHARS (LIST CONTROL-CHARS))))
  (WITH-OUTPUT-TO-STRING (STRING)
    (LOOP FOR CHAR = (PEEK-CHAR NIL STREAM NIL *EOF* NIL) UNTIL (OR (EOF-P CHAR) (MEMBER CHAR CONTROL-CHARS))
          DO (WRITE-CHAR (READ-CHAR STREAM) STRING))))
(DEFUN READ-INNER-LISP (STREAM CHAR) (WITH-MODE :INNER-LISP (READ-DELIMITED-LIST CHAR STREAM T)))
...
(DEFUN SUPPRESS-LINE-BREAK-READER (STREAM SUB-CHAR ARG)
  (DECLARE (IGNORE SUB-CHAR ARG))
  (WHEN (EQL (PEEK-CHAR NIL STREAM NIL NIL T) #\Newline) (READ-CHAR STREAM T NIL T))
  (VALUES))
(SET-CONTROL-CODE #\+ #'SUPPRESS-LINE-BREAK-READER :LISP)
(DEFUN START-SECTION-READER (STREAM SUB-CHAR ARG)
  (DECLARE (IGNORE STREAM ARG))
  (MAKE-INSTANCE (ECASE SUB-CHAR (#\Space 'SECTION) (#\* 'STARRED-SECTION))))
(DOLIST (SUB-CHAR '(#\Space #\*)) (SET-CONTROL-CODE SUB-CHAR #'START-SECTION-READER '(:LIMBO :TEX :LISP)))
...
(DEFCLASS START-CODE-MARKER (MARKER) ((NAME :READER SECTION-NAME :INITARG :NAME))
...
(DEFUN READ-CONTROL-TEXT (STREAM &OPTIONAL (EOF-ERROR-P T) (EOF-VALUE NIL) (RECURSIVE-P NIL))
  (WITH-MODE :RESTRICTED
    (APPLY #'CONCATENATE 'STRING
           (LOOP FOR TEXT = (SNARF-UNTIL-CONTROL-CHAR STREAM #\@) AS X =
                 (READ-PRESERVING-WHITESPACE STREAM EOF-ERROR-P EOF-VALUE RECURSIVE-P) COLLECT TEXT IF
                 (EQ X *END-CONTROL-TEXT*) DO (LOOP-FINISH) ELSE COLLECT X))))
      ...
      (PROG (FORM COMMENTARY CODE SECTION SECTIONS)
            ...
            (WITH-MODE :LIMBO
              (LOOP (MAYBE-PUSH (SNARF-UNTIL-CONTROL-CHAR STREAM #\@) COMMENTARY)
                    ...
                    (TYPECASE FORM (EOF (GO EOF)) (SECTION (GO COMMENTARY)) (T (PUSH FORM COMMENTARY)))))
            ...
            (WITH-MODE :LISP
              (LOOP (SETQ FORM (READ-PRESERVING-WHITESPACE STREAM NIL *EOF* NIL))
                    (TYPECASE FORM
                      ...
                      (SECTION (GO COMMENTARY))
                      (START-CODE-MARKER
                       (CERROR "Start a new unnamed section with no commentary." 'SECTION-LACKS-COMMENTARY
                               :STREAM STREAM)
                      ...
                      (NEWLINE-MARKER
                       (UNLESS (NULL CODE)
                         (COND ((NEWLINEP (CAR CODE)) (POP CODE) (PUSH *PAR* CODE)) (T (PUSH FORM CODE)))))
...
(DEFUN TANGLE-1 (FORM &KEY (EXPAND-NAMED-SECTIONS-P T))
  (FLET ((TANGLE-1 (FORM) (TANGLE-1 FORM :EXPAND-NAMED-SECTIONS-P EXPAND-NAMED-SECTIONS-P)))
    (TYPECASE FORM
...
(DEFUN LOAD-WEB-FROM-STREAM
    (STREAM PRINT &KEY (APPENDP T) &AUX (*READTABLE* *READTABLE*) (*PACKAGE* *PACKAGE*) (*EVALUATING* T))
  (DOLIST (FORM (TANGLE (UNNAMED-SECTION-CODE-PARTS (READ-SECTIONS STREAM :APPENDP APPENDP))) T)
    (IF PRINT
        (LET ((RESULTS (MULTIPLE-VALUE-LIST (EVAL FORM)))) (FORMAT T "~&; ~{~S~^, ~}~%" RESULTS))
      (EVAL FORM))))
(DEFUN LOAD-WEB
    (FILESPEC &KEY (VERBOSE *LOAD-VERBOSE*) (PRINT *LOAD-PRINT*) (IF-DOES-NOT-EXIST T)
  ...
  (IF (STREAMP FILESPEC)
    ...
    (WITH-OPEN-FILE (STREAM (MERGE-PATHNAMES FILESPEC (MAKE-PATHNAME :TYPE "CLW" :CASE :COMMON)) :DIRECTION
                     :INPUT :EXTERNAL-FORMAT EXTERNAL-FORMAT :IF-DOES-NOT-EXIST
                     (IF IF-DOES-NOT-EXIST :ERROR NIL))
      (LOAD-WEB-FROM-STREAM STREAM PRINT))))
(DEFUN LOAD-SECTIONS-FROM-TEMP-FILE (FILE APPENDP &AUX (TRUENAME (PROBE-FILE FILE)))
  "Load web sections from FILE, then delete it."
  (WHEN TRUENAME
    (UNWIND-PROTECT
        (WITH-OPEN-FILE (STREAM TRUENAME :DIRECTION :INPUT) (LOAD-WEB-FROM-STREAM STREAM T :APPENDP APPENDP))
...
(DEFUN TESTS-FILE-PATHNAME (OUTPUT-FILE TYPE &KEY (TESTS-FILE NIL TESTS-FILE-SUPPLIED-P) &ALLOW-OTHER-KEYS)
  (IF TESTS-FILE
    ...
    (UNLESS TESTS-FILE-SUPPLIED-P
      (MAKE-PATHNAME :NAME (CONCATENATE 'STRING (PATHNAME-NAME OUTPUT-FILE :CASE :COMMON) "-TESTS")
        :TYPE TYPE
...
(DEFUN TANGLE-FILE
    (INPUT-FILE &REST ARGS &KEY OUTPUT-FILE TESTS-FILE (VERBOSE *COMPILE-VERBOSE*) (PRINT *COMPILE-PRINT*)
     ...
     (TESTS-FILE (APPLY #'TESTS-FILE-PATHNAME OUTPUT-FILE "LISP" ARGS)) (*READTABLE* *READTABLE*)
     (*PACKAGE* *PACKAGE*))
  ...
  (LET ((UNUSED-SECTIONS 'NIL))
    (LABELS ((COLLECT-UNUSED-SECTIONS (SECTION)
      ...
      (MAP NIL
           (LAMBDA (SECTION)
             (WARN 'UNUSED-NAMED-SECTION-WARNING :FORMAT-CONTROL "Unused named section <~A>."
  ...
  (FLET ((WRITE-FORMS (SECTIONS OUTPUT-FILE)
    ...
    (WHEN (AND TESTS-FILE (> (LENGTH *TEST-SECTIONS*) 1))
      ...
      (COMPILE-FILE TESTS-FILE :VERBOSE VERBOSE :PRINT PRINT :EXTERNAL-FORMAT EXTERNAL-FORMAT))
...
(DEFUN WEAVE
    (INPUT-FILE &REST ARGS &KEY OUTPUT-FILE TESTS-FILE (VERBOSE *WEAVE-VERBOSE*) (PRINT *WEAVE-PRINT*)
     (IF-DOES-NOT-EXIST T) (EXTERNAL-FORMAT :DEFAULT) &AUX
     (INPUT-FILE (MERGE-PATHNAMES INPUT-FILE (MAKE-PATHNAME :TYPE "CLW" :CASE :COMMON)))
     (OUTPUT-FILE
      (IF OUTPUT-FILE
     ...
     (TESTS-FILE (APPLY #'TESTS-FILE-PATHNAME OUTPUT-FILE "TEX" ARGS)) (*READTABLE* *READTABLE*)
     (*PACKAGE* *PACKAGE*))
  ...
  (WHEN (AND TESTS-FILE (> (LENGTH *TEST-SECTIONS*) 1))
    ...
    (WEAVE-SECTIONS *TEST-SECTIONS* TESTS-FILE :PRINT PRINT :EXTERNAL-FORMAT EXTERNAL-FORMAT))
  (WHEN VERBOSE (FORMAT T "~&; weaving sections to ~A~%" OUTPUT-FILE))
  (WEAVE-SECTIONS *SECTIONS* OUTPUT-FILE :PRINT PRINT :EXTERNAL-FORMAT EXTERNAL-FORMAT))
(DEFUN WEAVE-SECTIONS (SECTIONS OUTPUT-FILE &KEY (PRINT *WEAVE-PRINT*) (EXTERNAL-FORMAT :DEFAULT))
  (WITH-OPEN-FILE (OUTPUT OUTPUT-FILE :DIRECTION :OUTPUT :EXTERNAL-FORMAT EXTERNAL-FORMAT :IF-EXISTS
                   :SUPERSEDE)
    ...
    (FLET ((WRITE-SECTION (SECTION)
      ...
      (IF PRINT
          (PPRINT-LOGICAL-BLOCK (NIL NIL :PER-LINE-PREFIX "; ")
                                (MAP NIL
                                     (LAMBDA (SECTION)
                                       (FORMAT T "~:[~;~:@_*~]~D~:_ " (TYPEP SECTION 'STARRED-SECTION)
...
(DEFUN PRINT-TEX (STREAM TEX-MODE-MATERIAL)
  (DOLIST (X TEX-MODE-MATERIAL)
    (ETYPECASE X
      ...
      (LIST (LET ((*INNER-LISP* T)) (DOLIST (FORM X) (FORMAT STREAM "\\(~W\\)" FORM)))))))
  ...
  (WITH-MODE :RESTRICTED
    (WITH-INPUT-FROM-STRING (STREAM INPUT-STRING)
      (LOOP FOR TEXT = (SNARF-UNTIL-CONTROL-CHAR STREAM #\|) FOR FORMS =
            (READ-PRESERVING-WHITESPACE STREAM NIL *EOF* NIL) IF (PLUSP (LENGTH TEXT)) COLLECT TEXT IF
            (EOF-P FORMS) DO (LOOP-FINISH) ELSE COLLECT FORMS))))
...
(DEFUN PRINT-SECTION (STREAM SECTION)
  (FORMAT STREAM "~&\\~:[M~;N{1}~]{~:[~;T~]~D}" (TYPEP SECTION 'STARRED-SECTION)
          (TYPEP SECTION 'TEST-SECTION) (SECTION-NUMBER SECTION))
  (LET* ((COMMENTARY (SECTION-COMMENTARY SECTION))
...
(DEFPARAMETER *TEX-ESCAPE-ALIST* (QUOTE ((" \\%&#$^_~<>" . #\\) ("{" . "$\\{$") ("}" . "$\\}$"))))
(DEFUN WRITE-STRING-ESCAPED
    (STRING &OPTIONAL STREAM (ESCAPE-CHARS *TEX-ESCAPE-ALIST*) &AUX
     (STREAM (CASE STREAM ((T) *TERMINAL-IO*) ((NIL) *STANDARD-OUTPUT*) (OTHERWISE STREAM))))
  (LOOP FOR CHAR ACROSS STRING AS ESCAPE = (CDR (ASSOC CHAR ESCAPE-CHARS :TEST #'FIND)) IF ESCAPE DO
        (ETYPECASE ESCAPE
...
(DEFUN PRINT-STRING (STREAM STRING)
  (LOOP FOR LAST = 0 THEN (1+ NEWLINE) FOR NEWLINE = (POSITION #\Newline STRING :START LAST) AS LINE =
        (SUBSEQ STRING LAST NEWLINE) DO (FORMAT STREAM "\\.{~:[~;\"~]" (ZEROP LAST))
        (WRITE-STRING-ESCAPED LINE STREAM
                              (LIST* '("{*}" . #\\) '("\\" . "\\\\\\\\") '("\"" . "\\\\\"")
                                     *TEX-ESCAPE-ALIST*))
        (FORMAT STREAM "~:[~;\"~]}" (NULL NEWLINE)) WHEN NEWLINE DO (FORMAT STREAM "\\cr~:@_") ELSE DO
        (LOOP-FINISH)))
(SET-WEAVE-DISPATCH 'STRING #'PRINT-STRING)
(DEFUN PRINT-CHAR (STREAM CHAR)
  (LET ((GRAPHICP (AND (GRAPHIC-CHAR-P CHAR) (STANDARD-CHAR-P CHAR))) (NAME (CHAR-NAME CHAR)))
    (WRITE-STRING "\\#\\CH{" STREAM)
    (WRITE-STRING-ESCAPED (IF (AND NAME (NOT GRAPHICP)) NAME (MAKE-STRING 1 :INITIAL-ELEMENT CHAR)) STREAM
...
(DEFUN PRINT-SYMBOL (STREAM SYMBOL)
  (LET ((GROUP-P
    ...
    (WRITE-STRING-ESCAPED (WRITE-TO-STRING SYMBOL :ESCAPE NIL :PRETTY NIL) STREAM)
    (WHEN GROUP-P (WRITE-STRING "}" STREAM))))
(SET-WEAVE-DISPATCH 'SYMBOL #'PRINT-SYMBOL)
(SET-WEAVE-DISPATCH '(EQL LAMBDA) (LAMBDA (STREAM OBJ) (DECLARE (IGNORE OBJ)) (WRITE-STRING "\\L" STREAM)) 1)
  ...
  (LABELS ((FIND-NEXT-NEWLINE (LIST) (MEMBER-IF #'NEWLINEP LIST :KEY #'CAR))
           (NEXT-LOGICAL-BLOCK (LIST)
             (DO* ((BLOCK 'NIL)
                  ...
                  ((OR (ENDP LIST) (AND (EQ LIST NEWLINE) NEXT-INDENT (< NEXT-INDENT BLOCK-INDENT)))
...
(SET-WEAVE-DISPATCH 'EMPTY-LIST-MARKER
                    (LAMBDA (STREAM OBJ) (DECLARE (IGNORE OBJ)) (WRITE-STRING "()" STREAM)))
...
(SET-WEAVE-DISPATCH 'COMMENT-MARKER
                    (LAMBDA (STREAM OBJ)
                      ...
                      (WRITE-STRING "}" STREAM)))
(SET-WEAVE-DISPATCH 'BACKQUOTE-MARKER (LAMBDA (STREAM OBJ) (FORMAT STREAM "\\`~W" (BACKQ-FORM OBJ))))
(SET-WEAVE-DISPATCH 'COMMA-MARKER
                    (LAMBDA (STREAM OBJ)
                      (FORMAT STREAM "\\CO{~@[~C~]}~W" (COMMA-MODIFIER OBJ) (COMMA-FORM OBJ))))
...
(SET-WEAVE-DISPATCH 'BIT-VECTOR-MARKER
                    (LAMBDA (STREAM OBJ)
                      (FORMAT STREAM "\\#~@[~D~]*~{~[0~;1~]~}"
                              (AND (SLOT-BOUNDP OBJ 'LENGTH) (SLOT-VALUE OBJ 'LENGTH))
...
(SET-WEAVE-DISPATCH 'RADIX-MARKER
                    (LAMBDA (STREAM OBJ)
                      (FORMAT STREAM "$~VR_{~2:*~D}$" (RADIX-MARKER-BASE OBJ) (MARKER-VALUE OBJ))))
(SET-WEAVE-DISPATCH 'STRUCTURE-MARKER
                    (LAMBDA (STREAM OBJ) (FORMAT STREAM "\\#S~W" (STRUCTURE-MARKER-FORM OBJ))))
(SET-WEAVE-DISPATCH 'READ-TIME-CONDITIONAL-MARKER
                    (LAMBDA (STREAM OBJ)
                      (FORMAT STREAM "\\#~:[--~;+~]\\RC{~S" (READ-TIME-CONDITIONAL-PLUSP OBJ)
...
(DEFMETHOD WALK-FORM ((WALKER WALKER) FORM &OPTIONAL ENV &AUX (ENV (ENSURE-PORTABLE-WALKING-ENVIRONMENT ENV))
  ...
  (FLET ((SYMBOL-MACRO-P (FORM ENV)
    ...
    (LOOP (COND ((SYMBOL-MACRO-P FORM ENV))
...
(DEFMETHOD WALK-AS-SPECIAL-FORM-P (WALKER CAR FORM ENV) (DECLARE (IGNORE WALKER CAR FORM ENV)) NIL)
...
(DEFMETHOD WALK-ATOMIC-FORM ((WALKER WALKER) FORM ENV &OPTIONAL (EVALP T)) (DECLARE (IGNORE ENV EVALP)) FORM)
(DEFMETHOD WALK-COMPOUND-FORM ((WALKER WALKER) CAR FORM ENV)
  (DECLARE (IGNORE CAR)) `(,(WALK-ATOMIC-FORM WALKER (CAR FORM) ENV NIL) ,@(WALK-LIST WALKER (CDR FORM) ENV)))
(DEFMETHOD WALK-FUNCTION-NAME ((WALKER WALKER) FUNCTION-NAME ENV)
  (TYPECASE FUNCTION-NAME
    ...
    ((CONS (EQL SETF) (CONS SYMBOL NULL)) `(SETF ,(WALK-ATOMIC-FORM WALKER (CADR FUNCTION-NAME) ENV NIL)))
...
(MACROLET ((WALK-AS-SPECIAL-FORM (OPERATOR)
             `(DEFMETHOD WALK-AS-SPECIAL-FORM-P ((WALKER WALKER) (CAR (EQL ',OPERATOR)) FORM ENV)
...
(DEFMACRO DEFINE-SPECIAL-FORM-WALKER
    (OPERATOR (WALKER FORM ENV &REST REST) &BODY BODY &AUX (CAR `(CAR (EQL ',OPERATOR))))
  (FLET ((ARG-NAME (ARG) (IF (CONSP ARG) (CAR ARG) ARG)))
    `(PROGN (DEFMETHOD WALK-AS-SPECIAL-FORM-P (,WALKER ,CAR ,FORM ,ENV)
...
(MACROLET ((DEFINE-BLOCK-LIKE-WALKER (OPERATOR)
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR ((WALKER WALKER) FORM ENV)
                `(,(CAR FORM) ,(CADR FORM) ,@(WALK-LIST WALKER (CDDR FORM) ENV)))))
...
(MACROLET ((DEFINE-QUOTE-LIKE-WALKER (OPERATOR)
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR ((WALKER WALKER) FORM ENV) (DECLARE (IGNORE WALKER ENV))
                FORM)))
...
(DEFUN WALK-LAMBDA-LIST (WALKER LAMBDA-LIST DECLS ENV &AUX NEW-LAMBDA-LIST (STATE :REQVARS))
  (LABELS ((AUGMENT-ENV (&REST VARS &AUX (VARS (REMOVE-IF #'NULL VARS)))
    ...
    (DO* ((LAMBDA-LIST LAMBDA-LIST (CDR LAMBDA-LIST)) (ARG (CAR LAMBDA-LIST) (CAR LAMBDA-LIST)))
      ...
      (ECASE STATE
        (:ENVVAR (PUSH (POP LAMBDA-LIST) NEW-LAMBDA-LIST) (UPDATE-STATE (CAR LAMBDA-LIST)))
        ((:REQVARS :RESTVAR)
         (ETYPECASE ARG
           (SYMBOL
...
(DEFUN WALK-LAMBDA-EXPRESSION (WALKER FORM ENV &AUX (LAMBDA-LIST (CADR FORM)) (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS DOC)
    ...
    (MULTIPLE-VALUE-BIND (LAMBDA-LIST ENV)
      ...
      `(,(WALK-FUNCTION-NAME WALKER (CAR FORM) ENV) ,LAMBDA-LIST ,@(IF DOC `(,DOC))
...
(DEFINE-SPECIAL-FORM-WALKER LET
  ((WALKER WALKER) FORM ENV &AUX
   (BINDINGS (MAPCAR (LAMBDA (P) (WALK-VARIABLE-BINDING WALKER P ENV)) (CADR FORM))) (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS
                   (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :VARIABLE (MAPCAR #'CAR BINDINGS) :DECLARE
                     DECLS)))))
(DEFINE-SPECIAL-FORM-WALKER FLET
  ((WALKER WALKER) FORM ENV &AUX
   (BINDINGS (MAPCAR (LAMBDA (P) (WALK-LAMBDA-EXPRESSION WALKER P ENV)) (CADR FORM))) (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS
                   (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :FUNCTION (MAPCAR #'CAR BINDINGS) :DECLARE
                     DECLS)))))
(DEFINE-SPECIAL-FORM-WALKER MACROLET
  ...
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :MACRO MACROS :DECLARE DECLS)))))
(DEFINE-SPECIAL-FORM-WALKER SYMBOL-MACROLET
  ...
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
    ...
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS
                   (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :SYMBOL-MACRO BINDINGS :DECLARE DECLS)))))
    ...
    `(,(CAR FORM)
      ,(MAPCAR (LAMBDA (P)
                 (LET ((WALKED-BINDING (WALK-VARIABLE-BINDING WALKER P ENV)))
                   (SETQ ENV
                         (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :VARIABLE (LIST (CAR WALKED-BINDING))
                           :DECLARE DECLS))
                   WALKED-BINDING))
               BINDINGS)
      ,@(IF DECLS `((DECLARE ,@DECLS))) ,@(WALK-LIST WALKER FORMS ENV))))
(DEFINE-SPECIAL-FORM-WALKER LABELS ((WALKER WALKER) FORM ENV &AUX (BINDINGS (CADR FORM)) (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
      (PARSE-BODY BODY :WALKER WALKER :ENV ENV)
    (LET* ((FUNCTION-NAMES (MAPCAR (LAMBDA (P) (WALK-FUNCTION-NAME WALKER (CAR P) ENV)) BINDINGS))
...
(DEFUN ENTRY-HEADING-LESSP (H1 H2 &AUX (H1 (IF (LISTP H1) H1 (LIST H1))) (H2 (IF (LISTP H2) H2 (LIST H2))))
...
(DEFCLASS INDEX-ENTRY (BINARY-SEARCH-TREE)
          ((KEY :ACCESSOR ENTRY-HEADING)
           (LOCATORS :ACCESSOR ENTRY-LOCATORS :INITARG :LOCATORS :INITFORM 'NIL)))
(DEFMETHOD FIND-OR-INSERT (ITEM (ROOT INDEX-ENTRY) &KEY (PREDICATE #'ENTRY-HEADING-LESSP) (TEST #'EQUALP)
                           (INSERT-IF-NOT-FOUND T))
  (CALL-NEXT-METHOD ITEM ROOT :PREDICATE PREDICATE :TEST TEST :INSERT-IF-NOT-FOUND INSERT-IF-NOT-FOUND))
...
(DEFMETHOD ADD-INDEX-ENTRY ((INDEX INDEX) HEADING (SECTION SECTION) &KEY DEFP)
  (FLET ((MAKE-LOCATOR NIL (MAKE-LOCATOR :SECTION SECTION :DEFP DEFP)))
...
(DEFUN MAKE-KEYWORD-FROM-DEF (SYMBOL)
  (VALUES (INTERN (LOOP WITH NAME = (SYMBOL-NAME SYMBOL) FOR PREFIX IN '("define-" "def") AS N =
                        (MIN (LENGTH PREFIX) (LENGTH NAME)) IF (STRING-EQUAL (SUBSEQ NAME 0 N) PREFIX) DO
                        (RETURN (SUBSEQ NAME N)) FINALLY (WARN "Non-defining symbol ~S" SYMBOL)
                        (RETURN NAME))
                  (FIND-PACKAGE "KEYWORD"))))
(DEFUN SUBSTITUTE-SYMBOLS (FORM SECTION &AUX SYMBOLS)
  (LABELS ((GET-SYMBOLS (FORM)
             (COND ((AND (SYMBOLP FORM) (MEMBER (SYMBOL-PACKAGE FORM) *INDEX-PACKAGES*))
...
(DEFMETHOD SECTION-CODE :AROUND ((SECTION SECTION))
  (LET ((CODE (CALL-NEXT-METHOD)))
    (IF *INDEXING* (SUBSTITUTE-SYMBOLS (TANGLE CODE :EXPAND-NAMED-SECTIONS-P NIL) SECTION) CODE)))
(DEFUN TANGLE-CODE-FOR-INDEXING (SECTIONS)
  (LET ((*INDEXING* T)) (TANGLE (UNNAMED-SECTION-CODE-PARTS SECTIONS))))
(DEFCLASS INDEXING-WALKER (WALKER) ((INDEX :ACCESSOR WALKER-INDEX :INITFORM (MAKE-INDEX))))
(DEFMETHOD WALK-DECLARATION-SPECIFIERS ((WALKER INDEXING-WALKER) DECLS ENV)
  (LOOP FOR (IDENTIFIER . DATA) IN DECLS IF (EQL IDENTIFIER 'SPECIAL) COLLECT
        `(SPECIAL ,@(WALK-LIST WALKER DATA ENV))))
(DEFMETHOD WALK-ATOMIC-FORM ((WALKER INDEXING-WALKER) FORM ENV &OPTIONAL (EVALP T))
  (IF (SYMBOLP FORM)
      ...
      (ADD-INDEX-ENTRY INDEX
                       (LIST VARIABLE
                             (ECASE TYPE
                               ...
                               (:SYMBOL-MACRO (IF LOCAL ':LOCAL-SYMBOL-MACRO ':SYMBOL-MACRO))
                               (:CONSTANT ':CONSTANT)))
                       SECTION
        :DEFP DEFP))))
  ...
  (MULTIPLE-VALUE-BIND (TYPE LOCAL)
    ...
    (WHEN (MEMBER TYPE '(:FUNCTION :MACRO))
      (ADD-INDEX-ENTRY INDEX
                       (LIST FUNCTION
                             (ECASE TYPE
                               (:FUNCTION (IF LOCAL ':LOCAL-FUNCTION ':FUNCTION))
