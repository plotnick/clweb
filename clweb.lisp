  ...
  (FLET ((FINISH-SECTION (SECTION COMMENTARY CODE)
           ...
           (WHEN (SECTION-NAME SECTION)
             (LET ((NAMED-SECTION (FIND-SECTION (SECTION-NAME SECTION)))
               ...
               (IF APPEND-P (PUSHNEW NUMBER (SEE-ALSO NAMED-SECTION))
                   (SETF (SEE-ALSO NAMED-SECTION) (LIST NUMBER)))))
           SECTION))
...
(DEFUN PRINT-TEX (STREAM TEX-MODE-MATERIAL)
  (DOLIST (X TEX-MODE-MATERIAL)
    (ETYPECASE X
      ...
      (LIST (DOLIST (FORM X) (WRITE FORM :STREAM STREAM))))))
...
(DEFUN PRINT-SECTION (STREAM SECTION)
  ...
  (PRINT-COMMENTARY STREAM SECTION)
  (WHEN (SECTION-NAME SECTION)
    (WRITE-STRING "\\Y" STREAM)
    (PRINT-SECTION-NAME STREAM (FIND-SECTION (SECTION-NAME SECTION)))
    (WRITE-STRING "${}\\E{}$" STREAM))
  (DOLIST (FORM (SECTION-CODE SECTION))
    (PPRINT-LOGICAL-BLOCK (STREAM FORM :PER-LINE-PREFIX "\\+" :SUFFIX "\\cr")
      (WRITE FORM :STREAM STREAM))))
(SET-WEAVE-DISPATCH 'LIMBO-SECTION #'PRINT-COMMENTARY 1)
...
(DEFTYPE SEMI-SPECIAL ()
  `(OR (SATISFIES SPECIAL-OPERATOR-P) (MEMBER COND LOOP ,@LAMBDA-LIST-KEYWORDS)
       (SATISFIES DEF-P)))
(DEFUN PRINT-SYMBOL
       (STREAM SYMBOL
        &OPTIONAL (SPECIALP (TYPEP SYMBOL 'SEMI-SPECIAL))
        &AUX (KEYWORDP (KEYWORDP SYMBOL)))
  (WRITE-STRING (COND (SPECIALP "\\&{") (KEYWORDP "\\:{") (T "\\\\{")) STREAM)
  (WRITE-ESCAPED-STRING STREAM (WRITE-TO-STRING SYMBOL :ESCAPE NIL :PRETTY NIL)
                        (LIST* '("*" . "\\/$\\ast$") '("&" . "\\AM ")
                               *TEX-ESCAPE-ALIST*))
  (WRITE-STRING "}" STREAM))
(SET-WEAVE-DISPATCH 'SYMBOL #'PRINT-SYMBOL)
(DEFUN MAYBE-PRINT-NEWLINE (STREAM OBJ)
  (WHEN (EQ OBJ *NEWLINE*)
    (WRITE-STRING "\\cr" STREAM)
...
(DEFUN PRINT-LIST (STREAM LIST)
  ...
  (PPRINT-LOGICAL-BLOCK (STREAM LIST :PER-LINE-PREFIX "&" :SUFFIX "\\)")
    (LOOP (PPRINT-EXIT-IF-LIST-EXHAUSTED)
          (LET ((NEXT (PPRINT-POP)))
            (UNLESS (MAYBE-PRINT-NEWLINE STREAM NEXT)
              (WRITE-CHAR #\  STREAM)
              (WRITE NEXT :STREAM STREAM))))))
(DEFUN PRINT-FUN-CALL (STREAM FORM)
  (WRITE-STRING "\\(" STREAM)
  (PPRINT-LOGICAL-BLOCK (STREAM FORM :PER-LINE-PREFIX "&" :SUFFIX "\\)")
    (PPRINT-EXIT-IF-LIST-EXHAUSTED)
    (LET ((OPERATOR (PPRINT-POP)))
      (PRINT-SYMBOL STREAM OPERATOR))
    (PPRINT-EXIT-IF-LIST-EXHAUSTED)
    (LET ((NEXT (PPRINT-POP)))
      (COND
       ((MAYBE-PRINT-NEWLINE STREAM NEXT)
        (LOOP (PPRINT-EXIT-IF-LIST-EXHAUSTED)
              (LET ((NEXT (PPRINT-POP)))
                (UNLESS (MAYBE-PRINT-NEWLINE STREAM NEXT)
                  (WRITE-CHAR #\  STREAM)
                  (WRITE NEXT :STREAM STREAM)))))
       (T (WRITE-CHAR #\  STREAM)
        (PPRINT-LOGICAL-BLOCK (STREAM (CDR FORM) :PER-LINE-PREFIX "&")
          (LOOP (PPRINT-EXIT-IF-LIST-EXHAUSTED)
                (LET ((NEXT (PPRINT-POP)))
                  (UNLESS (MAYBE-PRINT-NEWLINE STREAM NEXT)
                    (WRITE-CHAR #\  STREAM)
                    (WRITE NEXT :STREAM STREAM))))))))))
(PROGN (REQUIRE 'SB-INTROSPECT) (IMPORT 'FUNCTION-ARGLIST))
(DEFUN LAMBDA-LIST-BODY (OPERATOR)
  "Return the position of the &body parameter in OPERATOR's lambda-list."
            ...
            (OR (POSITION '&BODY LAMBDA-LIST)
                (LET ((REST (POSITION '&REST LAMBDA-LIST)))
                  (WHEN
                      (AND REST
                           (LET ((RESTARG
                                  (SYMBOL-NAME (ELT LAMBDA-LIST (1+ REST)))))
                             (STRING= RESTARG "BODY")))
                    REST)))))
      BODY)))
(DEFUN OPERATOR-WITH-BODY-P (OPERATOR)
  (AND (FBOUNDP OPERATOR) (LAMBDA-LIST-BODY OPERATOR)))
(DEFUN PRINT-FORM-WITH-BODY (STREAM LIST)
  (WRITE-STRING "\\(" STREAM)
  (PPRINT-LOGICAL-BLOCK (STREAM LIST :PER-LINE-PREFIX "&" :SUFFIX "\\)")
    (PPRINT-EXIT-IF-LIST-EXHAUSTED)
    (PRINT-SYMBOL STREAM (PPRINT-POP) T)
    (MACROLET ((PRINT-NEXT (INDENTATION)
                 `(PROGN
                   (PPRINT-EXIT-IF-LIST-EXHAUSTED)
                   (LET ((NEXT (PPRINT-POP)))
                     (COND
                      ...
                      (T (WRITE-CHAR #\  STREAM)
                       (WRITE NEXT :STREAM STREAM)))))))
      (DOTIMES (I (LAMBDA-LIST-BODY (CAR LIST))) (PRINT-NEXT "\\qquad"))
      (LOOP (PRINT-NEXT "\\quad")))))
...
(PROGN
 (IMPORT SB-INT:*BACKQ-TOKENS*)
 (IMPORT '(PPRINT-BACKQUOTE PPRINT-BACKQ-COMMA)))
(PROGN
