...
(DEFPACKAGE "CLWEB"
            ...
            (:EXPORT "TANGLE-FILE" "LOAD-WEB" "WEAVE"
             "LOAD-SECTIONS-FROM-TEMP-FILE" "*TANGLE-FILE-PATHNAME*"
             "*TANGLE-FILE-TRUENAME*" "*WEAVE-PRINT*" "*WEAVE-VERBOSE*"
             "*INDEX-LEXICAL-VARIABLES*" "AMBIGUOUS-PREFIX-ERROR"
             "SECTION-NAME-CONTEXT-ERROR" "SECTION-NAME-USE-ERROR"
             "SECTION-NAME-DEFINITION-ERROR" "UNUSED-NAMED-SECTION-WARNING")
            (:SHADOW "ENCLOSE" #+:ALLEGRO "FUNCTION-INFORMATION"
...
(DEFPARAMETER *WHITESPACE*
    #.(COERCE '(#\Space #\Tab #\Newline #\Newline #\Page #\Return) 'STRING))
(EVAL-WHEN (:COMPILE-TOPLEVEL :LOAD-TOPLEVEL :EXECUTE)
...
(DEFINE-CONDITION AMBIGUOUS-PREFIX-ERROR (ERROR)
                  ((PREFIX :READER AMBIGUOUS-PREFIX :INITARG :PREFIX)
                   (FIRST-MATCH :READER AMBIGUOUS-PREFIX-FIRST-MATCH :INITARG
                    :FIRST-MATCH)
                   (ALT-MATCH :READER AMBIGUOUS-PREFIX-ALT-MATCH :INITARG
                    :ALT-MATCH))
                  (:REPORT
...
(DEFINE-CONDITION SECTION-NAME-DEFINITION-ERROR (SECTION-NAME-CONTEXT-ERROR)
                  ...
                  (:REPORT
                   (LAMBDA (CONDITION STREAM)
                     (FORMAT STREAM
                             "Can't define a named section in Lisp mode: ~A"
...
(DEFINE-CONDITION SECTION-LACKS-COMMENTARY (PARSE-ERROR)
                  ...
                  (:REPORT
                   (LAMBDA (ERROR STREAM)
                     (LET* ((INPUT-STREAM
                             (DO ((STREAM
                                   (SECTION-LACKS-COMMENTARY-STREAM ERROR)))
                                 (NIL)
                               (TYPECASE STREAM
                                 (ECHO-STREAM
                                  (SETQ STREAM
...
~:[~;~:*at position ~D in file ~A.~]~:@>" POSITION
                               (OR PATHNAME INPUT-STREAM))))))
...
(DEFMETHOD FIND-OR-INSERT (ITEM (ROOT BINARY-SEARCH-TREE) &KEY (PREDICATE #'<)
                           (TEST #'EQL) (INSERT-IF-NOT-FOUND T))
  (FLET ((LESSP (ITEM NODE) (FUNCALL PREDICATE ITEM (NODE-KEY NODE)))
         (SAMEP (ITEM NODE) (FUNCALL TEST ITEM (NODE-KEY NODE))))
    (DO ((PARENT NIL NODE)
         (NODE ROOT
          (IF (LESSP ITEM NODE) (LEFT-CHILD NODE) (RIGHT-CHILD NODE))))
...
(DEFMETHOD MAP-BST #'(TREE BINARY-SEARCH-TREE)
  (MAP-BST FUNCTION (LEFT-CHILD TREE)) (FUNCALL FUNCTION TREE)
        ...
        (OR (DOLIST (CHILD (LIST (LEFT-CHILD NODE) (RIGHT-CHILD NODE)))
              (WHEN CHILD
                (MULTIPLE-VALUE-BIND (ALT PRESENT-P)
                    (CALL-NEXT-METHOD ITEM CHILD :PREDICATE PREDICATE :TEST
                     TEST :INSERT-IF-NOT-FOUND NIL)
                  (WHEN PRESENT-P
...
(DEFUN SQUEEZE (STRING)
  (LOOP WITH SQUEEZING = NIL FOR CHAR ACROSS (STRING-TRIM *WHITESPACE* STRING)
        ...
        COLLECT #\Space INTO CHARS ELSE COLLECT CHAR INTO CHARS ELSE UNLESS
...
(DEFUN FIND-SECTION (NAME &AUX (NAME (SQUEEZE NAME)))
  (IF (NULL *NAMED-SECTIONS*)
      (VALUES (SETQ *NAMED-SECTIONS*
                    (MAKE-INSTANCE 'NAMED-SECTION :NAME NAME))
              NIL)
...
(DEFUN READ-MAYBE-NOTHING-INTERNAL
  ...
  (MULTIPLE-VALUE-LIST (LET* ((NEXT-CHAR
                               (PEEK-CHAR NIL STREAM NIL NIL RECURSIVE-P))
                              (MACRO-FUN
                               (AND NEXT-CHAR
                                    (GET-MACRO-CHARACTER NEXT-CHAR))))
                         (COND (MACRO-FUN
                                (READ-CHAR STREAM)
                                (CALL-READER-MACRO-FUNCTION MACRO-FUN STREAM
                                  NEXT-CHAR))
                               (T
                                (FUNCALL READ STREAM EOF-ERROR-P EOF-VALUE
                                         RECURSIVE-P))))))
(DEFUN READ-MAYBE-NOTHING
    (STREAM &OPTIONAL (EOF-ERROR-P T) EOF-VALUE RECURSIVE-P)
  (READ-MAYBE-NOTHING-INTERNAL #'READ STREAM EOF-ERROR-P EOF-VALUE
    RECURSIVE-P))
...
(DEFUN CALL-READER-MACRO-FUNCTION (FN STREAM CHAR)
  (WITH-READ-BUFFER NIL (FUNCALL FN STREAM CHAR)))
  ...
  (APPLY #'CALL-NEXT-METHOD INSTANCE SLOT-NAMES
         (LIST* :PROXY
                (MAKE-ECHO-STREAM STREAM
                                  (MAKE-STRING-OUTPUT-STREAM :ELEMENT-TYPE (STREAM-ELEMENT-TYPE
                                                                            STREAM)))
                INITARGS)))
...
(DEFUN RELEASE-CHARPOS-STREAM (STREAM)
  (MULTIPLE-VALUE-BIND (CHARPOS-STREAM PRESENT-P)
    ...
    (COND (PRESENT-P
           (SETF (CHARPOS-PROXY-STREAM CHARPOS-STREAM) NIL)
...
(DEFMACRO WITH-CHARPOS-INPUT-STREAM ((VAR STREAM &KEY (CHARPOS 0)) &BODY BODY)
  `(LET ((,VAR
          (CHARPOS-PROXY-STREAM (MAKE-CHARPOS-INPUT-STREAM ,STREAM
                                  :CHARPOS ,CHARPOS))))
     (UNWIND-PROTECT (PROGN ,@BODY) (RELEASE-CHARPOS-STREAM ,VAR))))
(DEFMACRO WITH-CHARPOS-OUTPUT-STREAM
    ((VAR STREAM &KEY (CHARPOS 0)) &BODY BODY)
  `(LET ((,VAR
          (CHARPOS-PROXY-STREAM (MAKE-CHARPOS-OUTPUT-STREAM ,STREAM
                                  :CHARPOS ,CHARPOS))))
     (UNWIND-PROTECT (PROGN ,@BODY) (RELEASE-CHARPOS-STREAM ,VAR))))
(DEFMACRO WITH-REWIND-STREAM
    ((VAR STREAM &OPTIONAL (REWIND 'REWIND)) &BODY BODY &AUX (IN (GENSYM))
     (OUT (GENSYM)) (CLOSING (GENSYM)))
     ...
     (FLET ((,REWIND NIL
              (LET ((,IN
                ...
                (PROG1 (SETQ ,VAR (MAKE-CONCATENATED-STREAM ,IN ,VAR))
                       (PUSH ,VAR ,CLOSING) (PUSH ,IN ,CLOSING)))))
       (UNWIND-PROTECT (PROGN ,@BODY) (MAP NIL #'CLOSE ,CLOSING)))))
(DEFMACRO READ-WITH-ECHO
    ((STREAM OBJECT ECHOED) &BODY BODY &AUX (OUT (GENSYM)) (ECHO (GENSYM))
...
(DEFUN LIST-READER (STREAM CHAR)
  (DECLARE (IGNORE CHAR))
  (LOOP WITH LIST = 'NIL WITH CHARPOS-LIST = 'NIL FOR N UPFROM 0 AND NEXT-CHAR
        = (PEEK-CHAR T STREAM T NIL T) AS CHARPOS = (STREAM-CHARPOS STREAM)
        UNTIL (CHAR= #\) NEXT-CHAR) IF (CHAR= #\. NEXT-CHAR) DO
        ...
        (RETURN (MAKE-INSTANCE 'LIST-MARKER :LENGTH N :LIST (NREVERSE LIST)
...
(DOLIST (MODE '(:LISP :INNER-LISP))
  (SET-MACRO-CHARACTER #\' #'SINGLE-QUOTE-READER NIL
...
(DEFUN SHARPSIGN-QUOTE-READER (STREAM SUB-CHAR ARG)
  ...
  (MAKE-INSTANCE 'FUNCTION-MARKER :QUOTE 'FUNCTION :FORM
                 (READ STREAM T NIL T)))
(DOLIST (MODE '(:LISP :INNER-LISP))
...
(DEFUN SIMPLE-BIT-VECTOR-READER (STREAM SUB-CHAR ARG)
  ...
  (APPLY #'MAKE-INSTANCE 'BIT-VECTOR-MARKER :ELEMENTS
         (COERCE (LOOP FOR CHAR = (READ-CHAR STREAM NIL #\Space T) WHILE
                       (OR (CHAR= CHAR #\0) (CHAR= CHAR #\1)) COLLECT
                       (ECASE CHAR (#\0 0) (#\1 1)) FINALLY
...
(DEFUN RADIX-READER (STREAM SUB-CHAR ARG)
  (MAKE-INSTANCE 'RADIX-MARKER :BASE
                 (OR (CDR (ASSOC (CHAR-UPCASE SUB-CHAR) *RADIX-PREFIX-ALIST*))
                     ARG)
                 :VALUE
                 (FUNCALL (GET-DISPATCH-MACRO-CHARACTER #\# SUB-CHAR
...
(DEFMETHOD MARKER-VALUE ((MARKER ARRAY-MARKER))
  (LOOP WITH CONTENTS = (TANGLE (ARRAY-MARKER-INITIAL-CONTENTS MARKER)) REPEAT
        (ARRAY-MARKER-RANK MARKER) FOR SEQ = CONTENTS THEN (ELT SEQ 0) COLLECT
        (LENGTH SEQ) INTO DIMENSIONS FINALLY
...
(DEFMETHOD MARKER-VALUE ((MARKER STRUCTURE-MARKER))
  (LET ((*READTABLE* (READTABLE-FOR-MODE NIL)))
    (VALUES (READ-FROM-STRING
               (WRITE-TO-STRING MARKER :PPRINT-DISPATCH
                                *TANGLE-PPRINT-DISPATCH*
                 :PRETTY T
                 :READABLY T)))))
...
(DEFUN READ-INNER-LISP (STREAM CHAR)
  (WITH-MODE :INNER-LISP (READ-DELIMITED-LIST CHAR STREAM)))
...
(SET-MACRO-CHARACTER #\| (GET-MACRO-CHARACTER #\) NIL) NIL
                     (READTABLE-FOR-MODE :INNER-LISP))
(DOLIST (MODE *MODES*)
  (IGNORE-ERRORS (MAKE-DISPATCH-MACRO-CHARACTER #\@ T
                   (READTABLE-FOR-MODE MODE))))
...
(DEFUN START-TEST-SECTION-READER (STREAM SUB-CHAR ARG)
  ...
  (PROG1 (IF (AND (CHAR= (PEEK-CHAR T STREAM T NIL T) #\*)
         ...
         (LOOP UNTIL (CHAR/= (PEEK-CHAR T STREAM T NIL T) #\Newline) DO
               (READ-CHAR STREAM T NIL T))))
    ...
    (WITH-MODE :RESTRICTED
      (LOOP FOR TEXT = (SNARF-UNTIL-CONTROL-CHAR STREAM #\@) FOR NEXT =
            (READ-PRESERVING-WHITESPACE STREAM EOF-ERROR-P EOF-VALUE
              RECURSIVE-P)
            DO (WRITE-STRING TEXT STRING) IF (EQ NEXT *END-CONTROL-TEXT*) DO
            (LOOP-FINISH) ELSE DO (WRITE-STRING NEXT STRING)))))
  ...
  (WITH-CHARPOS-INPUT-STREAM (STREAM INPUT-STREAM)
    (FLET ((FINISH-SECTION (SECTION COMMENTARY CODE)
      ...
      (PROG (FORM COMMENTARY CODE SECTION SECTIONS)
            ...
            (WITH-MODE :LIMBO
              (LOOP (MAYBE-PUSH (SNARF-UNTIL-CONTROL-CHAR STREAM #\@)
                                COMMENTARY)
                    (LET ((NEXT-INPUT
                           (READ-MAYBE-NOTHING-PRESERVING-WHITESPACE STREAM
                             NIL *EOF* NIL)))
                      (WHEN NEXT-INPUT
...
(DEFUN UNNAMED-SECTION-CODE-PARTS (SECTIONS)
  (MAPAPPEND #'SECTION-CODE
             (COERCE (REMOVE-IF #'SECTION-NAME SECTIONS) 'LIST)))
  ...
  (DOLIST (FORM
           (TANGLE
              (UNNAMED-SECTION-CODE-PARTS
                (READ-SECTIONS STREAM :APPEND APPEND)))
           T)
    (IF PRINT
...
(DEFUN TESTS-FILE-PATHNAME
  ...
  (IF TESTS-FILE
    ...
    (UNLESS TESTS-FILE-SUPPLIED-P
      (MERGE-PATHNAMES (MAKE-PATHNAME :NAME (CONCATENATE
                                             'STRING
                                             (PATHNAME-NAME
                                              OUTPUT-FILE
                                              :CASE
...
(DEFUN TANGLE-FILE
  ...
  (SETQ *INDEX-PACKAGES* NIL)
  (SETQ *INDEX* (MAKE-INDEX))
  (WITH-OPEN-FILE (INPUT INPUT-FILE :DIRECTION :INPUT :EXTERNAL-FORMAT
                   EXTERNAL-FORMAT)
    (READ-SECTIONS INPUT))
  ...
  (FLET ((WRITE-FORMS (SECTIONS OUTPUT-FILE)
           (WITH-OPEN-FILE (OUTPUT OUTPUT-FILE :DIRECTION :OUTPUT :IF-EXISTS
                            :SUPERSEDE :EXTERNAL-FORMAT EXTERNAL-FORMAT)
    ...
    (MULTIPLE-VALUE-PROG1 (APPLY #'COMPILE-FILE LISP-FILE :ALLOW-OTHER-KEYS T
                                 ARGS)
      (WHEN TESTS-FILE
...
(DEFUN WEAVE-SECTIONS
    (SECTIONS &KEY INPUT-FILE OUTPUT-FILE INDEX-FILE SECTIONS-FILE TESTS
  ...
  (FLET ((WEAVE (OBJECT STREAM)
    ...
    (MACROLET ((WITH-OUTPUT-FILE ((STREAM FILESPEC) &BODY BODY)
                 `(WITH-OPEN-FILE (,STREAM ,FILESPEC :DIRECTION :OUTPUT
                                   :EXTERNAL-FORMAT EXTERNAL-FORMAT :IF-EXISTS
                                   :SUPERSEDE)
                    ,@BODY)))
         ...
         (WITH-OUTPUT-FILE (SCN SECTIONS-FILE)
          (MAP-BST (LAMBDA (SECTION)
                     (UNLESS (EVERY #'TEST-SECTION-P
                       ...
                       (WEAVE
                          (MAKE-INSTANCE 'SECTION-NAME-INDEX-ENTRY
...
(DEFUN PRINT-SECTION (STREAM SECTION)
  ...
  (LET* ((COMMENTARY (SECTION-COMMENTARY SECTION))
    ...
    (WHEN (AND (NOT NAMED-SECTION) (TYPEP SECTION 'TEST-SECTION)
...
(DEFUN PRINT-ESCAPED
  ...
  (LOOP FOR CHAR ACROSS STRING AS ESCAPE =
        (CDR (ASSOC CHAR *PRINT-ESCAPE-LIST* :TEST #'FIND)) IF ESCAPE DO
        (ETYPECASE ESCAPE
...
(DEFUN PRINT-STRING (STREAM STRING)
  (LOOP WITH *PRINT-ESCAPE-LIST* =
        `(("{*}" . #\\) ("\\" . "\\\\\\\\") ("\"" . "\\\\\"")
          ,@*PRINT-ESCAPE-LIST*)
        FOR LAST = 0 THEN
        (IF (CHAR= (ELT STRING (1- NEWLINE)) #\~)
            (POSITION-IF-NOT #'WHITESPACEP STRING :START NEWLINE)
          (1+ NEWLINE))
        FOR NEWLINE = (POSITION #\Newline STRING :START LAST) AS LINE =
        (SUBSEQ STRING LAST NEWLINE) DO
        (FORMAT STREAM "\\.{~:[~;\"~]~/clweb::print-escaped/~:[~;\"~]}"
...
(DEFUN PRINT-SYMBOL (STREAM SYMBOL)
  (LET* ((GROUP-P
          (COND ((MEMBER SYMBOL LAMBDA-LIST-KEYWORDS)
                 (WRITE-STRING "\\K{" STREAM))
                ((KEYWORDP SYMBOL) (WRITE-STRING "\\:{" STREAM))))
         (STRING (WRITE-TO-STRING SYMBOL :ESCAPE NIL :PRETTY NIL)))
    (MULTIPLE-VALUE-BIND (PREFIX SUFFIX)
        (LOOP WITH STRING-LENGTH = (LENGTH STRING) FOR (SUFFIX . REPLACEMENT)
              IN *PRINT-SYMBOL-SUFFIXES* AS PREFIX-END =
              (MAX 0 (- STRING-LENGTH (LENGTH SUFFIX))) WHEN
              (STRING= STRING SUFFIX :START1 PREFIX-END) DO
              (RETURN (VALUES (SUBSEQ STRING 0 PREFIX-END) REPLACEMENT))
...
(DEFUN PRINT-LOGICAL-BLOCK (STREAM BLOCK)
  ...
  (PPRINT-LOGICAL-BLOCK (STREAM (LOGICAL-BLOCK-LIST BLOCK) :PER-LINE-PREFIX
                        ...
                        (DO (INDENT NEXT (OBJ (PPRINT-POP) NEXT))
                            (NIL)
                          (COND ((NEWLINEP OBJ)
                                 (FORMAT STREAM "\\cr~:[~;\\Y~]~:@_"
                                         (TYPEP OBJ 'PAR-MARKER))
                                 (SETQ INDENT (INDENTATION OBJ))
                                 (PPRINT-EXIT-IF-LIST-EXHAUSTED)
                                 (SETQ NEXT (PPRINT-POP)))
                                (T
                                 (FORMAT STREAM "~@[~[~;\\1~;\\1~:;\\2~]~]~W"
                                         INDENT OBJ)
                                 (SETQ INDENT NIL)
                                 (PPRINT-EXIT-IF-LIST-EXHAUSTED)
                                 (SETQ NEXT (PPRINT-POP))
...
(DEFMETHOD WALK-AS-SPECIAL-FORM-P (WALKER OPERATOR FORM ENV)
  (DECLARE (IGNORE WALKER OPERATOR FORM ENV)) NIL)
...
(MACROLET ((WALK-AS-SPECIAL-FORM (OPERATOR)
             `(DEFMETHOD WALK-AS-SPECIAL-FORM-P ((WALKER WALKER)
...
(DEFMACRO DEFINE-SPECIAL-FORM-WALKER
    (OPERATOR (WALKER FORM ENV &REST REST) &BODY BODY &AUX
  ...
  (FLET ((ARG-NAME (ARG) (IF (CONSP ARG) (CAR ARG) ARG)))
    `(PROGN (DEFMETHOD WALK-AS-SPECIAL-FORM-P (,WALKER ,OPARG ,FORM ,ENV)
              (DECLARE
               (IGNORABLE ,@(MAPCAR #'ARG-NAME `(,WALKER ,FORM ,ENV))))
              T)
...
(MACROLET ((DEFINE-QUOTE-LIKE-WALKER (OPERATOR)
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR ((WALKER WALKER) FORM ENV)
                (DECLARE (IGNORE WALKER ENV)) FORM)))
...
(DEFUN PARSE-BODY (BODY &KEY DOC-STRING-ALLOWED WALKER ENV &AUX DOC)
  (FLET ((DOC-STRING-P (X REMAINING-FORMS)
    ...
    (LOOP FOR FORMS = BODY THEN (CDR FORMS) AS X = (CAR FORMS) WHILE FORMS IF
          (DOC-STRING-P X (CDR FORMS)) DO (SETQ DOC X) ELSE IF
          (DECLARATION-P X) APPEND (CDR X) INTO DECLS ELSE DO (LOOP-FINISH)
          FINALLY
          (RETURN (VALUES FORMS
                          (IF WALKER
  ...
  (LABELS ((AUGMENT-ENV (&REST VARS &AUX (VARS (REMOVE-IF #'NULL VARS)))
    ...
    (DO* ((LAMBDA-LIST LAMBDA-LIST (CDR LAMBDA-LIST))
           ...
           (SYMBOL
            (COND ((MEMBER ARG LAMBDA-LIST-KEYWORDS)
                  ...
                  (T
                   ...
                   (AUGMENT-ENV ARG)
                   (PUSH ARG NEW-LAMBDA-LIST))))
           (CONS (DESTRUCTURING-BIND (VAR/PATTERN &OPTIONAL INIT-FORM
                  ...
                  (SYMBOL
                   (COND ((MEMBER ARG LAMBDA-LIST-KEYWORDS)
                         ...
                         (T
                          ...
                          (AUGMENT-ENV ARG)
                          (PUSH ARG NEW-LAMBDA-LIST))))
                  (CONS (DESTRUCTURING-BIND (VAR/KV
                                             &OPTIONAL
                                             INIT-FORM
                                             SUPPLIED-P-PARAMETER)
                            ARG
                          (WHEN INIT-FORM
                            (SETQ INIT-FORM (WALK-FORM WALKER INIT-FORM ENV)))
                          (COND ((CONSP VAR/KV)
                                 (DESTRUCTURING-BIND (KEYWORD-NAME
                                                      VAR/PATTERN)
                                     VAR/KV
                                   (MULTIPLE-VALUE-SETQ (VAR/PATTERN ENV)
                                     (MAYBE-DESTRUCTURE VAR/PATTERN))
                                   (SETQ VAR/KV
                                         (LIST
                                          (WALK-VAR KEYWORD-NAME)
                                          VAR/PATTERN))))
                                (T
                                 (SETQ VAR/KV (WALK-VAR VAR/KV))
                                 (AUGMENT-ENV VAR/KV)))
                          (WHEN SUPPLIED-P-PARAMETER
                            (SETQ SUPPLIED-P-PARAMETER
                                  (WALK-VAR SUPPLIED-P-PARAMETER))
                            (AUGMENT-ENV SUPPLIED-P-PARAMETER))
      ...
      (WHEN (OR (ATOM LAMBDA-LIST)
        ...
        (LET ((VAR
               (WALK-VAR
                (IF (CONSP LAMBDA-LIST) (CDR LAMBDA-LIST) LAMBDA-LIST))))
          (AUGMENT-ENV VAR)
  ...
  (LET ((REQ-PARAMS
         (FLET ((AUGMENT-ENV (VAR)
           ...
           (LOOP UNTIL
                 ...
                 AS VAR = (WALK-VAR (POP LAMBDA-LIST)) DO
                 (AUGMENT-ENV (IF (CONSP VAR) (CAR VAR) VAR)) COLLECT VAR))))
...
(DEFUN WALK-LAMBDA-EXPRESSION
    (WALKER FORM ENV &REST ARGS &AUX (LAMBDA-LIST (CADR FORM))
     (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS DOC)
    ...
    (MULTIPLE-VALUE-BIND (LAMBDA-LIST ENV)
      ...
      `(,(APPLY #'WALK-FUNCTION-NAME WALKER (CAR FORM) ENV ARGS) ,LAMBDA-LIST
        ,@(IF DOC `(,DOC)) ,@(IF DECLS `((DECLARE ,@DECLS)))
...
(DEFUN WALK-VARIABLE-BINDING
    (WALKER P ENV &AUX (BINDING (IF (CONSP P) P (LIST P))))
  ...
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
    ...
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS
                   (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :VARIABLE
                     (MAPCAR #'CAR BINDINGS) :DECLARE DECLS)))))
(DEFINE-SPECIAL-FORM-WALKER FLET
  ((WALKER WALKER) FORM ENV &AUX
   (BINDINGS
    (MAPCAR (LAMBDA (P)
              (WALK-LAMBDA-EXPRESSION WALKER P ENV :OPERATOR 'FLET :LOCAL T
                :DEF T))
            (CADR FORM)))
   (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
...
(DEFUN MAKE-MACRO-DEFINITIONS (WALKER DEFS ENV)
  (MAPCAR (LAMBDA (DEF &AUX
                   (NAME
                    (WALK-ATOMIC-FORM WALKER :SYMBOL-MACRO-BINDING (CAR DEF)
                                      ENV)))
            (LIST NAME
                  (ENCLOSE (PARSE-MACRO NAME (CADR DEF) (CDDR DEF) ENV) ENV
                           WALKER)))
          DEFS))
(DEFINE-SPECIAL-FORM-WALKER MACROLET
  ((WALKER WALKER) FORM ENV &AUX
   (BINDINGS
    (MAPCAR (LAMBDA (P)
              (WALK-LAMBDA-EXPRESSION WALKER P ENV :OPERATOR 'MACROLET :LOCAL
                T :DEF T))
            (CADR FORM)))
   (BODY (CDDR FORM)))
  (MULTIPLE-VALUE-BIND (FORMS DECLS)
    ...
    `(,(CAR FORM) ,BINDINGS ,@(IF DECLS `((DECLARE ,@DECLS)))
      ,@(WALK-LIST WALKER FORMS
                   (AUGMENT-WALKER-ENVIRONMENT WALKER ENV :MACRO
                     (MAKE-MACRO-DEFINITIONS WALKER BINDINGS ENV) :DECLARE
                     DECLS)))))
...
(DEFUN INTERESTING-SYMBOL-P (OBJECT)
  (AND (SYMBOLP OBJECT) (MEMBER (SYMBOL-PACKAGE OBJECT) *INDEX-PACKAGES*)))
(DEFUN ENTRY-HEADING-LESSP
    (H1 H2 &AUX (H1 (ENSURE-LIST H1)) (H2 (ENSURE-LIST H2)))
  (OR (AND (NULL H1) H2)
      (STRING-LESSP (HEADING-NAME (CAR H1)) (HEADING-NAME (CAR H2)))
      (AND (STRING-EQUAL (HEADING-NAME (CAR H1)) (HEADING-NAME (CAR H2)))
           (CDR H2) (ENTRY-HEADING-LESSP (CDR H1) (CDR H2)))))
(DEFUN ENTRY-HEADING-EQUALP
    (H1 H2 &AUX (H1 (ENSURE-LIST H1)) (H2 (ENSURE-LIST H2)))
  (AND (= (LENGTH H1) (LENGTH H2))
       (EVERY #'STRING-EQUAL (MAPCAR #'HEADING-NAME H1)
              (MAPCAR #'HEADING-NAME H2))))
(DEFCLASS HEADING NIL ((NAME :INITARG :NAME)))
...
(DEFGENERIC HEADING-NAME (HEADING) (:METHOD-COMBINATION CONCATENATE-STRING)
            (:METHOD CONCATENATE-STRING ((HEADING HEADING))
             (SLOT-VALUE HEADING 'NAME))
            (:METHOD CONCATENATE-STRING ((HEADING SYMBOL)) HEADING)
            (:METHOD CONCATENATE-STRING ((HEADING STRING)) HEADING))
(DEFCLASS GLOBAL/LOCAL-HEADING (HEADING) ((LOCAL :INITARG :LOCAL))
...
(DEFCLASS SETF-FUNCTION-HEADING (FUNCTION-HEADING) NIL
          (:DEFAULT-INITARGS :NAME "setf function"))
(DEFCLASS MACRO-HEADING (GLOBAL/LOCAL-HEADING) NIL
          (:DEFAULT-INITARGS :NAME "macro"))
(DEFCLASS SYMBOL-MACRO-HEADING (GLOBAL/LOCAL-HEADING) NIL
...
(DEFCLASS SETF-METHOD-HEADING (METHOD-HEADING) NIL
          (:DEFAULT-INITARGS :NAME "setf method"))
(DEFCLASS VARIABLE-HEADING (HEADING)
          ((SPECIAL :INITARG :SPECIAL) (CONSTANT :INITARG :CONSTANT))
          (:DEFAULT-INITARGS :NAME "variable" :SPECIAL NIL :CONSTANT NIL))
(DEFCLASS CLASS-HEADING (HEADING) NIL (:DEFAULT-INITARGS :NAME "class"))
(DEFCLASS CONDITION-CLASS-HEADING (HEADING) NIL
          (:DEFAULT-INITARGS :NAME "condition class"))
(DEFMACRO DEFINE-HEADING-NAME-PREFIX (CLASS &REST SLOT-NAMES)
  (LET ((*PRINT-CASE* :DOWNCASE) (*PRINT-ESCAPE* NIL) (*PRINT-PRETTY* NIL))
    `(DEFMETHOD HEADING-NAME CONCATENATE-STRING ((HEADING ,CLASS))
       (WITH-SLOTS ,SLOT-NAMES
         HEADING
         (COND ,@(LOOP FOR SLOT-NAME IN SLOT-NAMES COLLECT
                       `(,SLOT-NAME ,(FORMAT NIL "~A " SLOT-NAME))))))))
...
(DEFUN MAKE-SUB-HEADING
    (OPERATOR &REST ARGS &KEY FUNCTION-NAME QUALIFIERS &ALLOW-OTHER-KEYS)
  (APPLY #'MAKE-INSTANCE
         (ECASE OPERATOR
...
(DEFMETHOD ADD-INDEX-ENTRY ((INDEX INDEX) HEADING (SECTION SECTION) &KEY DEF
                            &AUX (HEADING (ENSURE-LIST HEADING)))
      ...
      (ADD-INDEX-ENTRY INDEX
                       (LIST VARIABLE
                             (ECASE TYPE
                               ...
                               (:CONSTANT
                                (MAKE-INSTANCE 'VARIABLE-HEADING :CONSTANT T))
                               (:SYMBOL-MACRO
    ...
    (WHEN (MEMBER TYPE '(:FUNCTION :MACRO))
      (ADD-INDEX-ENTRY INDEX
                       (LIST FUNCTION-NAME
                             (ECASE TYPE
                               (:FUNCTION
                                (MAKE-INSTANCE 'FUNCTION-HEADING :LOCAL LOCAL
                                               :GENERIC
                                               (GENERIC-FUNCTION-P
                                                FUNCTION-NAME)))
                               (:MACRO
...
(DEFMETHOD WALK-COMPOUND-FORM :BEFORE
  ...
  (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
    ...
    (WHEN SECTION (INDEX-FUNCALL (WALKER-INDEX WALKER) SYMBOL SECTION ENV))))
...
(DEFMETHOD WALK-DECLARATION-SPECIFIERS ((WALKER INDEXING-WALKER) DECLS ENV)
  (LOOP FOR (IDENTIFIER . DATA) IN DECLS IF (EQL IDENTIFIER 'SPECIAL) COLLECT
        `(SPECIAL ,@(WALK-LIST WALKER DATA ENV)) ELSE IF
        (EQL IDENTIFIER 'OPTIMIZE) COLLECT `(OPTIMIZE ,@DATA)))
                ...
                (:METHOD
                 (LET* ((OPERATOR (POP FORM))
                   ...
                   (WALK-FUNCTION-NAME WALKER FUNCTION-NAME ENV
                     ...
                     :QUALIFIERS QUALIFIERS
                     :DEF T)
                   (WALK-METHOD-DEFINITION WALKER OPERATOR NIL QUALIFIERS
  ...
  (MULTIPLE-VALUE-BIND (BODY-FORMS DECLS DOC)
    ...
    (MULTIPLE-VALUE-BIND (LAMBDA-LIST ENV)
      ...
      `(,OPERATOR ,@(WHEN FUNCTION-NAME `(,FUNCTION-NAME)) ,@QUALIFIERS
        ,LAMBDA-LIST ,@(IF DOC `(,DOC)) ,@(IF DECLS `((DECLARE ,@DECLS)))
...
(MACROLET ((DEFINE-DEFCLASS-WALKER (OPERATOR)
             `(DEFINE-SPECIAL-FORM-WALKER ,OPERATOR
                ((WALKER INDEXING-WALKER) FORM ENV)
                (DESTRUCTURING-BIND (OPERATOR NAME SUPERS SLOT-SPECS &REST
                                     OPTIONS)
                    FORM
                  (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
                                     ...
                                     (ADD-INDEX-ENTRY (WALKER-INDEX WALKER)
                                                      (LIST
                                                       ...
                                                       (MAKE-SUB-HEADING
                                                        OPERATOR))
                                                      SECTION))
                                   (WALK-ATOMIC-FORM WALKER :SUPERCLASS-NAME
                                                     SYMBOL ENV)))
                               SUPERS)
                      ,(MAPCAR (LAMBDA (SPEC)
...
(DEFUN WALK-SLOT-SPECIFIER (WALKER SPEC ENV)
  (ETYPECASE SPEC
    ...
    (CONS (DESTRUCTURING-BIND (NAME &REST OPTIONS)
              SPEC
            (LOOP FOR (OPT-NAME OPT-VALUE) ON OPTIONS BY #'CDDR IF
                  (MEMBER OPT-NAME '(:READER :WRITER :ACCESSOR)) DO
                  (MULTIPLE-VALUE-BIND (SYMBOL SECTION)
...
(DEFUN PRINT-ENTRY-HEADING (STREAM HEADING &REST ARGS)
  (DECLARE (IGNORE ARGS))
  (PPRINT-LOGICAL-BLOCK (STREAM HEADING) (PPRINT-EXIT-IF-LIST-EXHAUSTED)
                        (LOOP (LET ((HEADING (PPRINT-POP)))
                                (COND ((SYMBOLP HEADING)
