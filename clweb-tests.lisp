...
(IN-PACKAGE "CLWEB")
(DEFMACRO DEFTEST (NAME FORM &REST VALUES) `(RT:DEFTEST ,NAME ,FORM ,@VALUES))
(DEFTEST MAYBE-PUSH
 (LET ((LIST 'NIL))
   (MAYBE-PUSH 'A LIST)
   (MAYBE-PUSH NIL LIST)
...
(DEFMACRO WITH-PATHNAME-TEST-VARIABLES (&BODY BODY &AUX (FIX-LPN (GENSYM)))
  `(FLET ((,FIX-LPN (LPN)
     ...
     (PROGV
         '(*WEB-PATHNAME-DEFAULTS* *LISP-PATHNAME-DEFAULTS*
           *TEX-PATHNAME-DEFAULTS* *INDEX-PATHNAME-DEFAULTS*
...
(DEFTEST CHARPOS-INPUT-STREAM
 (LET ((*TAB-WIDTH* 8))
   (WITH-CHARPOS-INPUT-STREAM (S
     ...
     (VALUES (STREAM-CHARPOS S) (READ-LINE S) (STREAM-CHARPOS S) (READ-CHAR S)
             (READ-CHAR S) (READ-CHAR S) (STREAM-CHARPOS S) (READ-CHAR S)
 ...
 (WITH-INPUT-FROM-STRING (S "abcdef")
   (WITH-REWIND-STREAM (R S)
     (VALUES (READ-CHAR R) (READ-CHAR R) (READ-CHAR R)
             (PROGN (REWIND) (READ-CHAR R)) (PROGN (REWIND) (READ-LINE R)))))
 #\a #\b #\c #\a "bcdef")
...
(DEFTEST (READ-EMPTY-LIST :INNER-LISP)
 (TYPEP (READ-FORM-FROM-STRING "()" :MODE :INNER-LISP) 'EMPTY-LIST-MARKER) T)
...
(DEFTEST READ-LIST-ERROR
 (HANDLER-CASE (READ-FORM-FROM-STRING "(. a)") (READER-ERROR NIL :ERROR))
 :ERROR)
...
(DEFTEST (BQ TOO-MANY-FORMS)
 (WITH-SAMPLE-NAMED-SECTIONS
  (LET ((HANDLED NIL))
    (HANDLER-BIND ((ERROR
                    (LAMBDA (CONDITION) (SETQ HANDLED T) (CONTINUE CONDITION))))
...
(DEFTEST (READ-TIME-EVAL 1)
 (LET* ((*READ-EVAL* T) (*EVALUATING* NIL))
   (WRITE-TO-STRING (MARKER-VALUE (READ-FORM-FROM-STRING "#.(+ 1 1)"))
                    :PPRINT-DISPATCH *TANGLE-PPRINT-DISPATCH* :PRETTY T))
 "#.(+ 1 1)")
...
(DEFTEST (TANGLE-1 2) (TANGLE-1 (READ-FORM-FROM-STRING "(:a :b :c)"))
 (:A :B :C) T)
(DEFTEST (TANGLE-1 3)
 (WITH-SAMPLE-NAMED-SECTIONS (TANGLE-1 (READ-FORM-FROM-STRING "@<foo@>")))
 (:FOO) T)
(DEFTEST TANGLE
 (WITH-SAMPLE-NAMED-SECTIONS
...
(DEFTEST (UPDATE-CONTEXT LOCAL-MACRO)
 (LET ((CONTEXT
        (UPDATE-CONTEXT 'FOO (MAKE-CONTEXT 'OPERATOR)
                        (AUGMENT-ENVIRONMENT
                         (ENSURE-PORTABLE-WALKING-ENVIRONMENT NIL) :MACRO
                         `((FOO
                            ,(LAMBDA (FORM ENV)
                               (DECLARE (IGNORE ENV))
                               FORM)))))))
...
(DEFTEST (UPDATE-CONTEXT LOCAL-FUNCTION)
 (LET ((CONTEXT
        (UPDATE-CONTEXT 'FOO (MAKE-CONTEXT 'OPERATOR)
                        (AUGMENT-ENVIRONMENT
                         (ENSURE-PORTABLE-WALKING-ENVIRONMENT NIL) :FUNCTION
                         '(FOO)))))
...
(DEFTEST WALK-COMPOUND-FORM
 (WALK-COMPOUND-FORM (MAKE-INSTANCE 'WALKER) :FOO '(:FOO 2 3) NIL) (:FOO 2 3))
...
(DEFTEST LAMBDA-EXPRESSION-TYPE
 (FLET ((LAMBDA-EXPRESSION-P (X)
   ...
   (AND (LAMBDA-EXPRESSION-P '(LAMBDA (X) X))
        (LAMBDA-EXPRESSION-P '(LAMBDA () T)) (LAMBDA-EXPRESSION-P '(LAMBDA ()))
...
(DEFTEST TOPLEVEL
 (LET ((WALKER (MAKE-INSTANCE 'TEST-WALKER))
   ...
   (FLET ((WALK (FORM TOPLEVEL)
     ...
     (VALUES (WALK '(ENSURE-TOPLEVEL) T) (WALK '(ENSURE-TOPLEVEL NIL) NIL)
             (WALK
              '(LET ()
                 (ENSURE-TOPLEVEL NIL))
              T)
    ...
    `(DEFTEST (WALK ,NAME)
      (LET* ((FORM ',FORM)
        ...
        ,(COND (RESULT `(TREE-EQUAL WALKED-FORM ',RESULT))
               ((NOT RESULT-SUPPLIED) '(TREE-EQUAL WALKED-FORM FORM)) (T T)))
      T)))
...
(DEFTEST (PARSE-BODY 1)
 (PARSE-BODY '("doc" (DECLARE (OPTIMIZE SPEED)) :FOO :BAR) :DOC-STRING-ALLOWED
             T)
 (:FOO :BAR) ((OPTIMIZE SPEED)) "doc")
(DEFTEST (PARSE-BODY 2)
 (PARSE-BODY
  '((DECLARE (OPTIMIZE SPEED)) "doc" (DECLARE (OPTIMIZE SPACE)) :FOO :BAR)
 ...
 (LET ((NAMES '(*FOO* BAR BAZ))
   ...
   (MULTIPLE-VALUE-BIND (WALKED-NAMES NEW-ENV)
     ...
     (AND (EQUAL WALKED-NAMES NAMES)
          (EQUAL
           (MAPCAR (LAMBDA (VAR) (VARIABLE-INFORMATION VAR NEW-ENV)) NAMES)
...
(DEFTEST (WALK DEFINE-SYMBOL-MACRO)
 (LET ((WALKER (MAKE-INSTANCE 'TEST-WALKER)) (NAME (GENSYM)))
   ...
   (NOT
    (NULL
     (WALK-FORM WALKER `(CHECK-BINDING ,NAME :VARIABLE :SYMBOL-MACRO NIL) NIL
                T))))
 T)
...
(DEFTEST METHOD-HEADING-NAME
 (VALUES (HEADING-NAME (MAKE-CONTEXT 'METHOD-NAME))
         (HEADING-NAME
          (MAKE-CONTEXT 'METHOD-NAME :QUALIFIERS '(:BEFORE :DURING :AFTER))))
...
(DEFTEST (COALESCE-LOCATORS 1)
 (MAPCAR
  (LAMBDA (SECTIONS)
    (MAPCAR #'LOCATION
            (COALESCE-LOCATORS
             (MAPCAR (LAMBDA (N) (MAKE-INSTANCE 'SECTION-LOCATOR :SECTION N))
                     SECTIONS))))
